...
public class EngineTestServer implements Runnable, EngineServiceEventInterface {
    ...
    // The engine service for sending events to the Apex engines and the EngDEp service for engine administration
    ...
    /**
     * The listener interface for receiving testApex events. The class that is interested in processing a testApex event implements this interface, and the
     * object created with that class is registered with a component using the component's {@code addTestApexListener} method. When the testApex event occurs,
    ...
    private final class TestApexListener implements ApexEventListener {
        ...
        public synchronized void onApexEvent(final ApexEvent apexEvent) {
            ...
            // Calculate the performance of the Apex engine service on this policy execution run and accumulate the total statistics
        ...
        private void checkResult(final ApexEvent result) {
            ...
            assert ((byte) result.get("TestEstablishCaseSelected") >= 0 && (byte) result.get("TestEstablishCaseSelected") <= 3);
            assert ((byte) result.get("TestDecideCaseSelected") >= 0 && (byte) result.get("TestDecideCaseSelected") <= 3);
