...
/**
 * A console for printing messages with functionality similar to loggers. The class provides a static instance for all parts of an application. The default
 * configuration is to not collect errors or warnings. The default types being activated are errors, warnings, and info messages.
 *
...
public final class Console {
    ...
    /**
     * Creates a new console. The constructor is private since the class provides static access to an instance. The default for types is verbose.
     */
    private Console() {
        types = TYPE_VERBOSE;
     ...
     * 
     * @param appName new application name, use <code>null</code> to reset the application name, a non-blank string for a new name, blank strings are ignored
     */
    ...
    /**
     * Configures the console. Use the configuration flags in combination for the required configuration. For instance, to collect errors and warnings use
     * <code>CONFIG_COLLECT_ERRORS | CONFIG_COLLECT_WARNINGS</code>.
     * 
    ...
    /**
     * Prints an error message with message and objects if {@link #TYPE_ERROR} is set; and increases the error count. Errors are collected (if configuration is
     * set) and the error counter is increased regardless of the console error type settings.
     * 
     * @param message the error message, using the same format as the SLF4J MessageFormatter, nothing done if <code>blank</code>
    ...
    /**
     * Prints a warning message with message and objects if {@link #TYPE_WARNING} is set; and increases the warning count. Warnings are collected (if
     * configuration is set) and the warning counter is increased regardless of the console warning type settings.
     * 
     * @param message the warning message, using the same format as the SLF4J MessageFormatter, nothing done if <code>blank</code>
     * @param objects the objects for substitution in the message
     */
