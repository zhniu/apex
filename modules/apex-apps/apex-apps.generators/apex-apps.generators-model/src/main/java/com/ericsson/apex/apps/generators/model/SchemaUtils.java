...
import com.ericsson.apex.plugins.context.schema.avro.AvroSchemaHelper;
...
public final class SchemaUtils {
    ...
    public static Schema getEventSchema(final AxEvent event) throws ApexEventException {
        final Schema skeletonSchema = Schema.createRecord(event.getKey().getName(), event.getNameSpace(), "com.ericsson.apex.model.eventmodel.events", false);
     ...
     */
    public static Schema getEventParameterSchema(final AxField parameter, final Map<String, Schema> preexistingParamSchemas) throws ApexEventException {
        final SchemaHelper schemaHelper = new SchemaHelperFactory().createSchemaHelper(parameter.getKey(), parameter.getSchema().getKey());
        ...
        try {
            ...
            throw new ApexEventException("failed to decode a schema for parameter " + parameter.getKey().getLocalName() + " of type "
                    + parameter.getSchema().getID() + " with Java type " + schemaHelper.getSchemaClass(), e);
        }
        final String schemaname = parameterSchema.getFullName();

        // Get the Avro schema for this parameter, we need to keep track of sub-schemas for records because Avro does not
        ...
        try {
            ...
            throw new ApexEventException("failed to decode a schema for parameter " + parameter.getKey().getLocalName() + " of type "
                    + parameter.getSchema().getID() + " using Schema type " + schemaname, e);
        }
        if (alreadyseen != null) {
            // logger.warn("parameter "+ parameter.getKey().getLocalName() + " of type " + parameter.getSchema().getID() + " tries to redfine AVRO type
            // "+schemaname+", but it was previously defined. This parameter will use the previously defined version because AVRO does not support redefinition
     ...
     * @throws ApexEventException in case of any error
     */
    public static void processSubSchemas(final Schema avroParameterSchema, final Map<String, Schema> map) throws ApexEventException {
        if (avroParameterSchema.getType() == Schema.Type.RECORD) {
            ...
            if (alreadyregistered != null && !avroParameterSchema.equals(alreadyregistered)) {
                throw new ApexEventException("Parameter attempts to redefine type " + schematypename + " when it has already been defined");
            }
