...
/**
 * The Interface ApexEngine is used to control the execution of a single Apex engine thread. This engine instance executes the policies in an
 * {@link AxPolicyModel}, which defines the policies that are executed by the engine and the context in which they execute. Many instances of an Apex engine may
 * run on the same Apex model, in which case they operate the same policy set in parallel over the same context. When the {@code handleEvent} method is passed
 * to the Apex engine, the engine executes the policy triggered by that event. A single Apex engine instance does not executed multiple policies in parallel, it
 * receives a trigger event and executes the policy for that event to completion before it is available to execute another policy.
 *
 * @author Liam Fallon (liam.fallon@ericsson.com)
 */
public interface ApexEngine {
    /** The amount of milliseconds to wait for the current Apex engine to timeout on engine stop requests. If the timeout is exceeded, the stop aborts. */
    ...
    /**
     * Update the Apex model to be used by the Apex engine. The engine must be in state "STOPPED" when the model is updated. The engine will replace the current
     * model with the incoming model if the model of the engine was previously updated and the value of common context is transferred if there is common context
    ...
    /**
     * This method passes an event to the Apex model to invoke a policy. If the event matches a policy, then that policy is executed.
