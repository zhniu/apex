...
public abstract class TaskSelectExecutor implements Executor<EnEvent, AxArtifactKey, AxState, ApexInternalContext> {
    ...
    public final void executePost(final boolean returnValue) throws StateMachineException {
        if (!returnValue) {
            LOGGER.warn("execute-post: task selection logic failed on state \"" + axState.getKey().getID() + "\"");
            throw new StateMachineException("execute-post: task selection logic failed on state \"" + axState.getKey().getID() + "\"");
        }
