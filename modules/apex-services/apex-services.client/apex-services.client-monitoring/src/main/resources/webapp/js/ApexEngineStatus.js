...
function createEngineStatusTable(id, startStopStatus) {
	...
	actionTD.html('<label class="ebSwitcher"><input type="checkbox" ' + checked + ' class="ebSwitcher-checkbox" /><div class="ebSwitcher-body"><div class="ebSwitcher-onLabel">Stopped</div><div class="ebSwitcher-switch"></div><div class="ebSwitcher-offLabel">Started</div></div></label>');
	...
	checkbox.change(function(event) {
		...
		ajax_get(restRootURL + "startstop", startStopCallback, this.engineURL.hostname, this.engineURL.port, {engineId: id.split("_")[1], startstop: startstop});
	}.bind(this));
...
/*
 * Check for any changes in the Engine Status Table data and its charts and update only where necessary 
 */
function setEngineStatusData(engineStatusData) {
	...
	for(var esd in engineStatusData) {
		...
		var data = [engineStatusData[esd].timestamp, id.split("_")[1], engineStatusData[esd].status, engineStatusData[esd].last_message, engineStatusData[esd].up_time, engineStatusData[esd].policy_executions];
		...
		if(lastPolicyDurationChart) {
			updateChart(lastPolicyDurationChart, JSON.parse(engineStatusData[esd].last_policy_duration), chartConfig.nodeColour);
		} else {
			...
			createChart(JSON.parse(engineStatusData[esd].last_policy_duration), lastPolicyDurationDiv, chartConfig.title, chartConfig.unit, chartConfig.lineStroke, chartConfig.nodeColour);
		...
		if(averagePolicyDurationChart) {
			updateChart(averagePolicyDurationChart, JSON.parse(engineStatusData[esd].average_policy_duration), chartConfig.nodeColour);
		} else {
			chartConfig = this.config.engineChart.averagePolicyDurationChart;
			...
			createChart(JSON.parse(engineStatusData[esd].average_policy_duration), averagePolicyDurationDiv, chartConfig.title, chartConfig.unit, chartConfig.lineStroke, chartConfig.nodeColour);
