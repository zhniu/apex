...
function createChart(data, container, title, unit, lineStroke, nodeColour) {
    ...
    y.domain([0, d3.max(data, function(d) { return Math.ceil((d.value+1)/10)*10; })]);
    ...
    .on("mouseover", function(d) {		
    	d3.select("body").select(".tooltip").transition()		
            .duration(50)		
            .style("opacity", 1);		
    	d3.select("body").select(".tooltip").html(formatDate(new Date(d.timestamp)) + "<br/>"  + d.value + (unit ? " " + unit : ""))
...
/*
 * Generates random chart data. Used when initializing the charts so that they are not empty on load
 */
...
function updateChart(container, data, nodeColour) {
    ...
    svg.select("svg").select("g").selectAll("circle")
    .data(data)
  .enter().append("circle")
    .attr("r", 3.5)
    .attr("class", "circle")
