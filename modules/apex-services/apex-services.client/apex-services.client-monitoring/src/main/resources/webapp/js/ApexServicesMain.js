...
function servicesCallback(data) {
	...
	if(localStorage.getItem("apex-monitor-services")) {
		...
		setTimeout(function(){ this.servicesCall = ajax_get(restRootURL, servicesCallback, this.engineURL.hostname, this.engineURL.port); }, config.refresh);
	}
}
...
function setUpPage() {
	...
	// Clear each div
	$('#content > div').each(function () { $(this).empty();});
...
/*
 * Retrieves the engine URL from the cookie. If it has not been set yet, then a dialog is shown asking for it
 */
function getEngineURL(message) {
	// The engine URL is stored in a cookie using the key "apex-monitor-services"
	var engineURL = localStorage.getItem("apex-monitor-services");
	
	// This url is used to store the last known engine URL so that the user doesn't have to retype it every time
...
$(document).ready(function() {
	restRootURL = location.protocol + "//" + window.location.hostname + (location.port ? ':'+location.port : '') + (location.pathname.endsWith("/monitoring/") ? location.pathname.substring( 0, location.pathname.indexOf("monitoring/")) : location.pathname) + "apexservices/monitoring/";
