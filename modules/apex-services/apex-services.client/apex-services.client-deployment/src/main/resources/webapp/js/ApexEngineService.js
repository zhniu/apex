...
function createEngineServiceTable() {
	...
	var table = createEngineTable($("." + config.engineService.parent), tableId,
	...
	actionTD.html('<input type="text" name="period" id="period" style="display:inline-block"><label class="ebSwitcher"><input type="checkbox" class="ebSwitcher-checkbox" /><div class="ebSwitcher-body"><div class="ebSwitcher-onLabel">Stopped</div><div class="ebSwitcher-switch"></div><div class="ebSwitcher-offLabel">Started</div></div></label>');
	...
	checkbox.change(function(event) {
		...
		ajax_get(restRootURL + "periodiceventstartstop", startStopCallback, this.engineURL.hostname, this.engineURL.port, {engineId: this.engineId, startstop: startstop, period: period.val()}, resetPeriodicEvents);
	}.bind(this));
	$(table).children("#engineTableBody").append(tableRow);
}

/*
 * Check for any changes in the Engine Service Table data and update only where necessary 
 */
