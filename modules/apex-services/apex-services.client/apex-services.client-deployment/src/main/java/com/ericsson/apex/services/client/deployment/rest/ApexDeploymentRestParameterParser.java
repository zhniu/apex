...
public class ApexDeploymentRestParameterParser {
	...
	public ApexDeploymentRestParameters parse(final String[] args) {
		...
		try {
			...
			throw new ApexDeploymentRestParameterException("invalid command line arguments specified : " + e.getMessage());
		}
		...
		if (commandLine.getArgs().length > 0) {
			throw new ApexDeploymentRestParameterException("too many command line arguments specified : " + Arrays.toString(remainingArgs));
		}
		...
		try {
			...
			throw new ApexDeploymentRestParameterException("error parsing argument \"time-to-live\" :" + e.getMessage(), e);
		}
