...
public class ApexDeploymentRestMain {
    ...
    public static void main(String[] args) {
        try {
            ApexDeploymentRestMain restMain = new ApexDeploymentRestMain(args, System.out);
            restMain.init();
        } catch (Exception e) {
     ...
     */
    public ApexDeploymentRestMain(String[] args, final PrintStream outStream) {
            // Save the streams for output and error
            this.outStream = outStream;
            
            // Client parameter parsing
            ApexDeploymentRestParameterParser parser = new ApexDeploymentRestParameterParser();

            try {
                    // Get and check the parameters
                    parameters = parser.parse(args);
            }
            catch (ApexDeploymentRestParameterException e) {
                    throw new ApexDeploymentRestParameterException("Apex Services REST endpoint ("+this.toString()+") parameter error, " + e.getMessage() + '\n' + parser.getHelp(ApexDeploymentRestMain.class.getCanonicalName()));
            }

            if (parameters.isHelpSet()) {
                    throw new ApexDeploymentRestParameterException(parser.getHelp(ApexDeploymentRestMain.class.getCanonicalName()));
            }

            // Validate the parameters
            String validationMessage = parameters.validate();
            if (validationMessage.length() > 0) {
                    throw new ApexDeploymentRestParameterException("Apex Services REST endpoint ("+this.toString()+") parameters invalid, " + validationMessage + '\n' + parser.getHelp(ApexDeploymentRestMain.class.getCanonicalName()));
            }
