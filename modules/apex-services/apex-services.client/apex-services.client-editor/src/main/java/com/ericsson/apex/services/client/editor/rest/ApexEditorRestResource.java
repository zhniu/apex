...
import com.ericsson.apex.services.client.editor.rest.bean.BeanContextAlbum;
import com.ericsson.apex.services.client.editor.rest.bean.BeanContextSchema;
import com.ericsson.apex.services.client.editor.rest.bean.BeanEvent;
import com.ericsson.apex.services.client.editor.rest.bean.BeanField;
import com.ericsson.apex.services.client.editor.rest.bean.BeanKeyRef;
import com.ericsson.apex.services.client.editor.rest.bean.BeanLogic;
import com.ericsson.apex.services.client.editor.rest.bean.BeanModel;
import com.ericsson.apex.services.client.editor.rest.bean.BeanPolicy;
import com.ericsson.apex.services.client.editor.rest.bean.BeanState;
import com.ericsson.apex.services.client.editor.rest.bean.BeanStateOutput;
import com.ericsson.apex.services.client.editor.rest.bean.BeanStateTaskRef;
import com.ericsson.apex.services.client.editor.rest.bean.BeanTask;
import com.ericsson.apex.services.client.editor.rest.bean.BeanTaskParameter;
 ...
 * with a JSON string payload containing the new
 * {@code Schema} in the body, can be explained as:
 * <ul>
 * <li>The server or servlet is running at the base URL {@code http://localhost:8080/apex}
 * <li>This resource {@code ApexRestEditorResource} is used because the path {@code rest/109} matches the {@code Path} filter specification for this Resource
 * ({@code @Path("rest/{session}")}), where the {@code int} path parameter {@code session} is assigned the {@code int} value {@code 109}
 * <li>The path {@code ContextSchema/Update} redirects this call to the method {@link #updateContextSchema(String)}, which should be a {@link javax.ws.rs.PUT},
 * with a single String in the body/payload which gets mapped to the single String parameter for the method.
 * <li>So, in summary, the REST request updates a {@code ContextSchema} as specified in the payload for {@code session} number {@code 109}
 * </ul>
 * 
 ...
 * 
 * <b>Note:</b> To create a new {@code Session}, and have a new session ID allocated use {@link javax.ws.rs.GET} request to
 * {@code <baseURL>/rest/-1/Session/Create} (for example: {@code http://localhost:8080/apex/rest/-1/Session/Create} )
 *
 */
@Path("editor/{session}")
...
public class ApexEditorRestResource {
    ...
    /**
     * This method commits changes to the Apex model for the current editor session. This should only be called once, at the end of a successful change to the
    ...
    private ApexAPIResult commitChanges() {
        ...
        if (sessionApexModel == null) {
            return new ApexAPIResult(RESULT.FAILED,
                    "Cannot commit a changes for Session ID  \"" + sessionID + "\", because it has not been initialised / started");
        }
    ...
    /**
     * Creates a new session. Always call this method with sessionID -1, whereby a new sessionID will be allocated.
     * If successful the new sessionID will be available in the first message in the result.
     *
     * @return an ApexAPIResult object. If successful then {@link ApexAPIResult#isOK()} will return true.
     * Any messages/errors can be retrieved using {@link ApexAPIResult#getMessages()}. This includes the session id for this session.
     */
    @GET
    @Path("Session/Create")
     ...
     * The returned value(s) will be similar to {@link AxPolicyModel}, with merged {@linkplain AxKeyInfo} for the root object.
     * @return an ApexAPIResult object. If successful then {@link ApexAPIResult#isOK()} will return true.
     * Any messages/errors can be retrieved using {@link ApexAPIResult#getMessages()}
     */
    @PUT
    @Path("/Model/Load")
    ...
    /**
     * Analyse the model and return analysis results.
     * If successful the analysis results will be available in the messages in the result.
     *
     * @return an ApexAPIResult object. If successful then {@link ApexAPIResult#isOK()} will return true.
     * Any messages/errors can be retrieved using {@link ApexAPIResult#getMessages()}
     */
     ...
     * If successful the validation results will be available in the messages in the result.
     *
     * @return an ApexAPIResult object. If successful then {@link ApexAPIResult#isOK()} will return true.
     * Any messages/errors can be retrieved using {@link ApexAPIResult#getMessages()}
     */
    ...
    public ApexAPIResult createModel(final String jsonString) {
        ...
        try {
            ...
            ret = sessionApexModel.createModel(jsonbean.getName(), jsonbean.getVersion(), jsonbean.getUuid(), jsonbean.getDescription());
    ...
    public ApexAPIResult updateModel(final String jsonString) {
        ...
        try {
            ...
            final BeanModel jsonbean = RestUtils.getJSONParameters(jsonString, BeanModel.class);
            ret = sessionApexModel.updateModel(jsonbean.getName(), jsonbean.getVersion(), jsonbean.getUuid(), jsonbean.getDescription());
            if (ret.isOK()) {
                commitChanges();
            }
            return ret;
    ...
    /**
     * Gets the key for the model for this session.
     * If successful the model key will be available in the first message in the result. See {@linkplain AxKey}
     *
     * @return an ApexAPIResult object. If successful then {@link ApexAPIResult#isOK()} will return true.
     * Any messages/errors can be retrieved using {@link ApexAPIResult#getMessages()}
     */
    ...
    /**
     * Retrieve the model for this session.
     * If successful the model will be available in the first message in the result.
     * The returned value will be similar to a {@link AxPolicyModel}, with merged {@linkplain AxKeyInfo} for the root object.
     *
     * @return an ApexAPIResult object. If successful then {@link ApexAPIResult#isOK()} will return true.
     * Any messages/errors can be retrieved using {@link ApexAPIResult#getMessages()}
     */
    ...
    /**
     * List key information with the given key names/versions. If successful the result(s) will be available in the result messages. See {@linkplain AxKeyInfo}
     *
    ...
    public ApexAPIResult listKeyInformation(@QueryParam("name") final String name, @QueryParam("version") final String version) {
        ApexAPIResult ret = null;
        String name1 = name;
    ...
    public ApexAPIResult createContextSchema(final String jsonString) {
        ...
        try {
            ...
            ret = sessionApexModel.createContextSchema(jsonbean.getName(), jsonbean.getVersion(), jsonbean.getSchemaFlavour(), jsonbean.getSchemaDefinition(),
    ...
    public ApexAPIResult updateContextSchema(final String jsonString) {
        ...
        try {
            ...
            ret = sessionApexModel.updateContextSchema(jsonbean.getName(), jsonbean.getVersion(), jsonbean.getSchemaFlavour(), jsonbean.getSchemaDefinition(),
                    jsonbean.getUuid(), jsonbean.getDescription());
            if (ret.isOK()) {
                commitChanges();
            }
    ...
    /**
     * List context schemas with the given key names/versions.
     * If successful the result(s) will be available in the result messages.
     * The returned value(s) will be similar to {@link AxContextSchema}, with merged {@linkplain AxKeyInfo} for the root object.
     *
     * @param name the name to search for. If null or empty, then all names will be queried
     * @param version the version to search for. If null then all versions will be searched for.
     * @return an ApexAPIResult object. If successful then {@link ApexAPIResult#isOK()} will return true.
    ...
    public ApexAPIResult listContextSchemas(@QueryParam("name") final String name, @QueryParam("version") final String version) {
        ApexAPIResult ret = null;
        String name1 = name;
        String version1 = version;
        LOGGER.entry(name1, version1);
    ...
    public ApexAPIResult deleteContextSchema(@QueryParam("name") final String name, @QueryParam("version") final String version) {
        ApexAPIResult ret = null;
        String name1 = name;
        String version1 = version;
        LOGGER.entry(name1, version1);
    ...
    /**
     * Validate context schemas with the given key names/versions. The result(s) will be available in the result messages.
     *
     * @param name the name to search for. If null or empty, then all names will be queried
     * @param version the version to search for. If null then all versions will be searched for.
     * @return an ApexAPIResult object. If successful then {@link ApexAPIResult#isOK()} will return true.
    ...
    public ApexAPIResult validateContextSchemas(@QueryParam("name") final String name, @QueryParam("version") final String version) {
        ApexAPIResult ret = null;
        String name1 = name;
        String version1 = version;
        LOGGER.entry(name1, version1);
    ...
    public ApexAPIResult createContextAlbum(final String jsonString) {
        ...
        try {
            ...
            ret = sessionApexModel.createContextAlbum(jsonbean.getName(), jsonbean.getVersion(), jsonbean.getScope(), Boolean.toString(jsonbean.getWriteable()),
                    jsonbean.getItemSchema().getName(), jsonbean.getItemSchema().getVersion(), jsonbean.getUuid(), jsonbean.getDescription());
            if (ret.isOK()) {
                commitChanges();
            }
            return ret;
    ...
    public ApexAPIResult updateContextAlbum(final String jsonString) {
        ...
        try {
            ...
            ret = sessionApexModel.updateContextAlbum(jsonbean.getName(), jsonbean.getVersion(), jsonbean.getScope(), Boolean.toString(jsonbean.getWriteable()),
                    jsonbean.getItemSchema().getName(), jsonbean.getItemSchema().getVersion(), jsonbean.getUuid(), jsonbean.getDescription());
            if (ret.isOK()) {
                commitChanges();
            }
    ...
    /**
     * List context albums with the given key names/versions.
     * If successful the result(s) will be available in the result messages.
     * The returned value(s) will be similar to {@link AxContextAlbum}, with merged {@linkplain AxKeyInfo} for the root object.
     *
     * @param name the name to search for. If null or empty, then all names will be queried
     * @param version the version to search for. If null then all versions will be searched for.
     * @return an ApexAPIResult object. If successful then {@link ApexAPIResult#isOK()} will return true.
    ...
    public ApexAPIResult listContextAlbums(@QueryParam("name") final String name, @QueryParam("version") final String version) {
        ApexAPIResult ret = null;
        String name1 = name;
        String version1 = version;
        LOGGER.entry(name1, version1);
    ...
    public ApexAPIResult deleteContextAlbum(@QueryParam("name") final String name, @QueryParam("version") final String version) {
        ApexAPIResult ret = null;
        String name1 = name;
        String version1 = version;
        LOGGER.entry(name1, version1);
    ...
    /**
     * Validate context albums with the given key names/versions. The result(s) will be available in the result messages.
     *
     * @param name the name to search for. If null or empty, then all names will be queried
     * @param version the version to search for. If null then all versions will be searched for.
     * @return an ApexAPIResult object. If successful then {@link ApexAPIResult#isOK()} will return true.
    ...
    public ApexAPIResult validateContextAlbums(@QueryParam("name") final String name, @QueryParam("version") final String version) {
        ApexAPIResult ret = null;
        String name1 = name;
        String version1 = version;
        LOGGER.entry(name1, version1);
    ...
    public ApexAPIResult createEvent(final String jsonString) {
        ...
        try {
            ...
            ret = sessionApexModel.createEvent(jsonbean.getName(), jsonbean.getVersion(), jsonbean.getNameSpace(), jsonbean.getSource(), jsonbean.getTarget(),
                    jsonbean.getUuid(), jsonbean.getDescription());
            if (ret.isNOK()) {
                return ret;
            }
            if (jsonbean.getParameters() != null) {
                for (final Entry<String, BeanField> p : jsonbean.getParameters().entrySet()) {
                    if (p.getValue() == null) {
                        ret = new ApexAPIResult(RESULT.FAILED, "Null event parameter information for parameter \"" + p.getKey() + "\" in event "
                    ...
                    final ApexAPIResult rettmp = sessionApexModel.createEventPar(jsonbean.getName(), jsonbean.getVersion(), p.getKey(), p.getValue().getName(),
                            p.getValue().getVersion(), p.getValue().getOptional());
                    if (rettmp.isNOK()) {
                        rettmp.addMessage("Failed to add event parameter information for parameter \"" + p.getKey() + "\" in event " + jsonbean.getName() + ":"
                                + jsonbean.getVersion()
                                + ". The event was created, but there was an error adding the event parameters. The event has only been partially defined.");
                        ret = rettmp;
                        return ret;
    ...
    public ApexAPIResult updateEvent(final String jsonString) {
        ...
        try {
            ...
            if (jsonbean.getName() == null || jsonbean.getName().equals("") || jsonbean.getVersion() == null || jsonbean.getVersion().equals("")) {
                ret = new ApexAPIResult(RESULT.FAILED,
                        "Null/Empty event name/version (\"" + jsonbean.getName() + ":" + jsonbean.getVersion() + "\" passed to UpdateEvent");
                return ret;
            }
    ...
    /**
     * List events with the given key names/versions.
     * If successful the result(s) will be available in the result messages.
     * The returned value(s) will be similar to {@link AxEvent}, with merged {@linkplain AxKeyInfo} for the root object.
     *
     * @param name the name to search for. If null or empty, then all names will be queried
     * @param version the version to search for. If null then all versions will be searched for.
     * @return an ApexAPIResult object. If successful then {@link ApexAPIResult#isOK()} will return true.
    ...
    public ApexAPIResult createTask(final String jsonString) {
        ...
        try {
            ...
            ret = sessionApexModel.createTask(jsonbean.getName(), jsonbean.getVersion(), jsonbean.getUuid(), jsonbean.getDescription());
            if (ret.isNOK()) {
                return ret;
            }
            if (jsonbean.getInputFields() != null) {
                for (final Entry<String, BeanField> fin : jsonbean.getInputFields().entrySet()) {
                    if (fin.getValue() == null) {
                        ret = new ApexAPIResult(RESULT.FAILED,
                                "Null task input field information for field \"" + fin.getKey() + "\" in task " + jsonbean.getName() + ":"
                                        + jsonbean.getVersion()
                    ...
                    if (fin.getKey() == null || !fin.getKey().equals(fin.getValue().getLocalName())) {
                        ret = new ApexAPIResult(RESULT.FAILED,
                                "Invalid task input field information for field \"" + fin.getKey() + "\" in task " + jsonbean.getName() + ":"
                                        + jsonbean.getVersion()
                                        + ". The localName of the field (\"" + fin.getValue().getLocalName() + "\") is not the same as the field name. "
                                        + "The task was created, but there was an error adding the input fields. The task has only been partially defined.");
                        return ret;
                    }
            ...
            if (jsonbean.getOutputFields() != null) {
                for (final Entry<String, BeanField> fout : jsonbean.getOutputFields().entrySet()) {
                    if (fout.getValue() == null) {
                        ret = new ApexAPIResult(RESULT.FAILED,
                                "Null task output field information for field \"" + fout.getKey() + "\" in task " + jsonbean.getName() + ":"
                                        + jsonbean.getVersion()
                    ...
                    if (fout.getKey() == null || !fout.getKey().equals(fout.getValue().getLocalName())) {
                        ret = new ApexAPIResult(RESULT.FAILED,
                                "Invalid task output field information for field \"" + fout.getKey() + "\" in task " + jsonbean.getName() + ":"
                                        + jsonbean.getVersion()
                                        + ". The localName of the field (\"" + fout.getValue().getLocalName() + "\") is not the same as the field name. "
                                        + "The task was created, but there was an error adding the output fields. The task has only been partially defined.");
                        return ret;
                    }
            ...
            if (jsonbean.getTaskLogic() != null) {
                ...
                tempres = sessionApexModel.createTaskLogic(jsonbean.getName(), jsonbean.getVersion(), logic.getLogicFlavour(), logic.getLogic());
                if (tempres.isNOK()) {
                    tempres.addMessage("Failed to add task logic in task " + jsonbean.getName() + ":" + jsonbean.getVersion()
            ...
            if (jsonbean.getParameters() != null) {
                for (final Entry<String, BeanTaskParameter> param : jsonbean.getParameters().entrySet()) {
                    if (param.getKey() == null || param.getValue() == null || !param.getKey().equals(param.getValue().getParameterName())) {
                        ret = new ApexAPIResult(RESULT.FAILED,
                                "Null or invalid task parameter information for parameter \"" + param.getKey() + "\" in task " + jsonbean.getName() + ":"
                                        + jsonbean.getVersion()
                                        + ". The task was created, but there was an error adding the parameters. The task has only been partially defined.");
                        return ret;
                    }
                    tempres = sessionApexModel.createTaskParameter(jsonbean.getName(), jsonbean.getVersion(), param.getValue().getParameterName(),
                            param.getValue().getDefaultValue());
                    if (tempres.isNOK()) {
                        tempres.addMessage("Failed to add task parameter \"" + param.getKey() + "\" in task " + jsonbean.getName()
                        + ":" + jsonbean.getVersion()
                        + ". The task was created, but there was an error adding the parameters. The task has only been partially defined.");
                        ret = tempres;
                        return ret;
            ...
            if (jsonbean.getContexts() != null) {
                for (final BeanKeyRef contextalbum : jsonbean.getContexts()) {
                    if (contextalbum.getName() == null || contextalbum.getVersion() == null) {
                        ret = new ApexAPIResult(RESULT.FAILED, "Null or invalid context album reference information in task " + jsonbean.getName() + ":"
                                + jsonbean.getVersion()
                                + ". The task was created, but there was an error adding the"
                                + " context album reference. The task has only been partially defined.");
                        return ret;
                    }
                    tempres = sessionApexModel.createTaskContextRef(jsonbean.getName(), jsonbean.getVersion(), contextalbum.getName(),
                            contextalbum.getVersion());
                    if (tempres.isNOK()) {
                        ret = new ApexAPIResult(RESULT.FAILED, "Failed to add context album reference information in task " + jsonbean.getName() + ":"
                                + jsonbean.getVersion()
                                + ". The task was created, but there was an error adding the"
                                + " context album reference. The task has only been partially defined.");
                        return ret;
    ...
    public ApexAPIResult updateTask(final String jsonString) {
        ...
        try {
            ...
            if (jsonbean.getName() == null || jsonbean.getName().equals("") || jsonbean.getVersion() == null || jsonbean.getVersion().equals("")) {
                ret = new ApexAPIResult(RESULT.FAILED,
                        "Null/Empty task name/version (\"" + jsonbean.getName() + ":" + jsonbean.getVersion() + "\" passed to UpdateTask");
                return ret;
            }
    ...
    /**
     * List tasks with the given key names/versions.
     * If successful the result(s) will be available in the result messages.
     * The returned value(s) will be similar to {@link AxTask}, with merged {@linkplain AxKeyInfo} for the root object.
     *
     * @param name the name to search for. If null or empty, then all names will be queried
     * @param version the version to search for. If null then all versions will be searched for.
     * @return an ApexAPIResult object. If successful then {@link ApexAPIResult#isOK()} will return true.
    ...
    public ApexAPIResult deleteTask(@QueryParam("name") final String name, @QueryParam("version") final String version) {
        ...
        try {
            ...
            // all input/output fields, parameters, logic, context references is "owned"/contained in the task, so deleting the task removes all of these
    ...
    public ApexAPIResult createPolicy(final String jsonString) {
        ...
        try {
            ...
            ret = sessionApexModel.createPolicy(policyname, policyversion, jsonbean.getTemplate(), jsonbean.getFirstState(), jsonbean.getUuid(),
            ...
            if (jsonbean.getStates() == null || jsonbean.getStates().isEmpty()) {
                ret = new ApexAPIResult(RESULT.FAILED,
                        "Null or empty state map; no states defined for policy \"" + policyname + ":" + policyversion
            ...
            for (final Map.Entry<String, BeanState> e : statemap.entrySet()) {
                ...
                if (state == null) {
                    ret = new ApexAPIResult(RESULT.FAILED,
                            "Null or invalid state information for state \"" + statename + "\" for policy \"" + policyname + ":" + policyversion
                ...
                if (state.getTrigger() == null) {
                    ret = new ApexAPIResult(RESULT.FAILED,
                            "Null or invalid state trigger for state \"" + statename + "\" for policy \"" + policyname + ":" + policyversion
                            + "\". The policy was created, but there was an error adding the state. The policy has only been partially defined.");
                    return ret;
                }
                if (state.getDefaultTask() == null) {
                    ret = new ApexAPIResult(RESULT.FAILED,
                            "Null or invalid default task for state \"" + statename + "\" for policy \"" + policyname + ":" + policyversion
                            + "\". The policy was created, but there was an error adding the state. The policy has only been partially defined.");
                    return ret;
                }
                tempres = sessionApexModel.createPolicyState(policyname, policyversion, statename, state.getTrigger().getName(),
                        state.getTrigger().getVersion(), state.getDefaultTask().getName(), state.getDefaultTask().getVersion());
                if (tempres.isNOK()) {
                    ret = tempres;
                    return ret;
                }
            ...
            for (final Map.Entry<String, BeanState> e : statemap.entrySet()) {
                ...
                if (tsl != null) {
                    tempres = sessionApexModel.createPolicyStateTaskSelectionLogic(policyname, policyversion, statename, tsl.getLogicFlavour(), tsl.getLogic());
                    if (tempres.isNOK()) {
                ...
                if (contexts != null) {
                    for (final BeanKeyRef c : contexts) {
                        ...
                        tempres = sessionApexModel.createPolicyStateContextRef(policyname, policyversion, statename, c.getName(), c.getVersion());
                        if (tempres.isNOK()) {
                ...
                if (finalizers != null) {
                    for (final Map.Entry<String, BeanLogic> f : finalizers.entrySet()) {
                        ...
                        if (finalizername == null || finalizer == null) {
                            ret = new ApexAPIResult(RESULT.FAILED, "Null or invalid finalizer information for finalizer "
                                    + "named \"" + finalizername + "\" in state \""
                        ...
                        tempres = sessionApexModel.createPolicyStateFinalizerLogic(policyname, policyversion, statename, finalizername, finalizer.getLogicFlavour(),
                                finalizer.getLogic());
                        if (tempres.isNOK()) {
                            tempres.addMessage("Failed to add finalizer information for finalizer named \"" + finalizername + "\" in"
                ...
                if (outputs == null || outputs.isEmpty()) {
                    ret = new ApexAPIResult(RESULT.FAILED, "No state outputs have been defined in state \"" + statename + "\" for policy \"" + policyname + ":" + policyversion
                ...
                for (final Map.Entry<String, BeanStateOutput> o : outputs.entrySet()) {
                    ...
                    if (outputname == null || output == null || output.getEvent() == null) {
                        ret = new ApexAPIResult(RESULT.FAILED, "Null or invalid output information for output named \""
                                + outputname + "\" in state \"" + statename + "\" for policy \"" + policyname + ":" + policyversion
                                + "\". The policy and state were created, but there was an error adding the output."
                                + " The policy has only been partially defined.");
                        return ret;
                    }
                    tempres = sessionApexModel.createPolicyStateOutput(policyname, policyversion, statename, outputname, output.getEvent().getName(),
                            output.getEvent().getVersion(), output.getNextState());
                    if (tempres.isNOK()) {
                        tempres.addMessage("Failed to add output information for output named \"" + outputname + "\" in state \"" + statename
                                + "\" for policy \"" + policyname + ":" + policyversion
                                + "\". The policy and state were created, but there was an error adding the output."
                                + " The policy has only been partially defined.");
                        ret = tempres;
                ...
                if (taskmap == null || taskmap.isEmpty()) {
                    ret = new ApexAPIResult(RESULT.FAILED, "No tasks have been defined in state \"" + statename + "\" for policy \"" + policyname + ":" + policyversion
                ...
                for (final Map.Entry<String, BeanStateTaskRef> t : taskmap.entrySet()) {
                    ...
                    if (tasklocalname == null || taskref == null || taskref.getTask() == null) {
                        ret = new ApexAPIResult(RESULT.FAILED, "Null or invalid task information for task named \""
                                + tasklocalname + "\" in state \"" + statename + "\" for for policy \"" + policyname + ":" + policyversion
                                + "\". The policy and state were created, but there was an error adding the task. "
                                + "The policy has only been partially defined.");
                        return ret;
                    }
                    tempres = sessionApexModel.createPolicyStateTaskRef(policyname, policyversion, statename, tasklocalname, taskref.getTask().getName(),
                            taskref.getTask().getVersion(), taskref.getOutputType(), taskref.getOutputName());
                    if (tempres.isNOK()) {
                        tempres.addMessage("Failed to add task reference \"" + t + "\" for state \"" + statename + "\" for policy \""
                                + policyname + ":" + policyversion
    ...
    @PUT
    @Path("Policy/Update")
    public ApexAPIResult updatePolicy(@QueryParam("firstStatePeriodic") final boolean firstStatePeriodic, final String jsonString) {
        ApexAPIResult ret = null;
        LOGGER.entry(jsonString);
        try {
            ret = initialiseSessionForChanges();
            ...
            if (jsonbean.getName() == null || jsonbean.getName().equals("") || jsonbean.getVersion() == null || jsonbean.getVersion().equals("")) {
                ret = new ApexAPIResult(RESULT.FAILED,
                        "Null/Empty Policy name/version (\"" + jsonbean.getName() + ":" + jsonbean.getVersion() + "\" passed to UpdatePolicy");
                return ret;
            }
            ...
            if(firstStatePeriodic) {
                ApexAPIResult existingPeriodicEvent = sessionApexModel.listEvent("PeriodicEvent", null);
                if(existingPeriodicEvent.isNOK()) {
                    String periodicEventString = "{\"name\":\"PeriodicEvent\",\"version\":\"0.0.1\",\"uuid\":\"44236da1-3d47-4988-8033-b6fee9d6a0f4\",\"description\":\"Generated description for concept referred to by key 'PeriodicEvent:0.0.1'\",\"source\":\"System\",\"target\":\"Apex\",\"nameSpace\":\"com.ericsson.apex.domains.aadm.events\",\"parameters\":{}}";
    ...
    /**
     * List policies with the given key names/versions.
     * If successful the result(s) will be available in the result messages.
     * The returned value(s) will be similar to {@link AxPolicy}, with merged {@linkplain AxKey Info} for the root object.
     *
     * @param name the name to search for. If null or empty, then all names will be queried
     * @param version the version to search for. If null then all versions will be searched for.
     * @return an ApexAPIResult object. If successful then {@link ApexAPIResult#isOK()} will return true.
    ...
    /**
     * The json strings representing the objects listed, stored in result.messages[], does not contain the AxKeyInformation for that object. This utility method
     * retrieves the AxKeyInfo for each object and adds it to the json for the object.
     * 
    ...
    private ApexAPIResult addKeyInfo2Messages(final ApexAPIResult result) {
        ...
        for (final String message : messages) {
            try {
                ...
                else if (jsonObject != null && jsonObject.get("policyKey") != null && jsonObject.get("policyKey").isJsonObject()
                ...
                if (name == null && version == null && jsonObject.entrySet() != null && jsonObject.entrySet().size() > 0) {
                    objecttochange = (JsonObject) jsonObject.entrySet().iterator().next().getValue();
                    if (objecttochange != null && objecttochange.get("key") != null && objecttochange.get("key").isJsonObject()
                            && objecttochange.getAsJsonObject("key").get("name") != null) {
                        name = objecttochange.getAsJsonObject("key").get("name").getAsString();
                    }
                    else if (objecttochange != null && objecttochange.get("policyKey") != null && objecttochange.get("policyKey").isJsonObject()
                ...
                if (name != null && version != null) {
                    ...
                    if (keyInfoResult.isOK() && keyInfoMessages != null && keyInfoMessages.size() > 0) {
                        ...
                        if (keyInfoJsonObject != null && keyInfoJsonObject.get("apexKeyInfo") != null
                            ...
                            desc = keyInfoJsonObject.get("apexKeyInfo").getAsJsonObject().get("description").getAsString();
                        }
                    }
                }
    ...
    protected static int createCorruptSession() {
        ApexEditorRestResource apexEditorRestResource = new ApexEditorRestResource();
        ApexAPIResult result = apexEditorRestResource.createSession();
        int corruptSessionId = new Integer(result.getMessages().get(0));
