...
import com.ericsson.apex.services.client.editor.rest.bean.BeanBase;
...
public abstract class RestUtils {
    ...
    public static Map<String, String> getJSONParameters(final String jsonString) {
        ...
        for (final Entry<String, JsonElement> jsonEntry : jsonObject.entrySet()) {
            jsonMap.put(jsonEntry.getKey(), (jsonEntry.getValue() == JsonNull.INSTANCE ? null : blank2null(jsonEntry.getValue().getAsString())));
        }
        return jsonMap;
    }
    ...
    private static final String JSON_INPUT_TYPE_REGEXP = "^\\s*[\\(\\{\\[][\\s+\\S]*[\\)\\}\\]]"; // starts with some kind of bracket [ or ( or {, then has
                                                                                                  // something, then has and has a close bracket
     ...
     */
    public static <CLZ extends AxConcept> CLZ getConceptFromJSON(final String jsonString, final Class<CLZ> clz) throws JAXBException {
