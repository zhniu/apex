function editPolicyForm_State_generateStateDiv(createEditOrView, policy, statename, state, tasks, events, contextAlbums, contextItemSchemas){
	...
	if(state != null && state.trigger != null){
		eventselected =  {"name":state.trigger.name,"version":state.trigger.version,"displaytext":state.trigger.name+":"+state.trigger.version};
	}
	var ineventSelectDiv = dropdownList("editPolicyFormTrigger"+"_"+statename, events, eventselected, (createEditOrView == "VIEW"), function(){return editPolicyForm_updateTriggerEventOptions(events)});
...
//	contextstable_head_tr.appendChild(document.createElement("th")); //empty, for delete button
...
//	contextstable_head_th.setAttribute("class", "table-policycontext-heading");
	...
	if(state && state.contextAlbumReference && $.isArray(state.contextAlbumReference)){
		for (var p = 0; p < state.contextAlbumReference.length; p++) {
			...
			var ce = {"name":contextEntry.name,"version":contextEntry.version,"displaytext":contextName};
			editPolicyForm_State_addPolicyContext(contextstable_body, (createEditOrView == "VIEW"), statename, contextName, ce, contextAlbums);
		}
	}
...
//		addContextInput.setAttribute("value", "Add New Policy Logic Context");
//		addContextInput.innerHTML = addContextInput.getAttribute("value");
		addContextInput.onclick = function(){return editPolicyForm_State_addPolicyContext(contextstable_body, false, statename, null, null, contextAlbums);};
	}
	...
	taskstable_head_tr.appendChild(document.createElement("th")); //empty, for delete button
	...
	if(state && state.taskReferences && $.isArray(state.taskReferences.entry)){
		for (var p = 0; p < state.taskReferences.entry.length; p++) {
			...
			var taskselected = {"name":taskEntry.key.name,"version":taskEntry.key.version,"displaytext":taskName};
...
//		addTaskInput.innerHTML = addTaskInput.getAttribute("value");
		addTaskInput.onclick = function(){return editPolicyForm_State_addPolicyTask(taskstable_body, false, false, state, statename, null, null, tasks);};
	}
	...
	var textarea = showHideTextarea("editEventFormTaskSelLogicInput"+"_"+statename, logic, false, !edit_disabled, false);
	...
	if (state && state.taskSelectionLogic && state.taskSelectionLogic.logicFlavour!= null && state.taskSelectionLogic.logicFlavour != "" && state.taskSelectionLogic.logicFlavour.toUpperCase() != "UNDEFINED"){
	...
	dir_outputstable_head_tr.appendChild(document.createElement("th")); //empty, for delete button
	...
	stateNextStateOptions.push({"name":"NULL","displaytext":"None","state":null}); 
	if(policy && policy.state && policy.state.entry && $.isArray(policy.state.entry)){
		for (var s = 0; s < policy.state.entry.length; s++) {
			...
			if(statename != st.key){ //state cannot have itself as nextstate 
				stateNextStateOptions.push({"name":st.key,"displaytext":st.key,"state":st.value}); 
			}
		}
	} 
	if(state && state.stateOutputs && $.isArray(state.stateOutputs.entry)){
		for (var p = 0; p < state.stateOutputs.entry.length; p++) {
			...
			if( nxtst != null && nxtst.toUpperCase() != "NULL"){
				nextState = {"name":nxtst ,"displaytext":nxtst };
			}
			if( nxtst != null && nxtst.toUpperCase() == "NULL"){
				nextState = {"name":"NULL","displaytext":"None"};
			}
			var outEvent = null; 
			var oute =  outputEntry.value.outgoingEvent;
			if( oute != null && oute.name != null && oute.version != null){
				outEvent = {"name":oute.name, "version":oute.version,"displaytext":oute.name+":"+oute.version};
			}
			editPolicyForm_State_addStateDirectOutput(dir_outputstable_body, (createEditOrView == "VIEW"), statename, state, outputName,  nextState, outEvent, stateNextStateOptions, events);
		}
	}
...
//		addStateDirOutput.setAttribute("id",    "editPolicyFormDirOutputsAddOutput"+"_"+statename);
//		addStateDirOutput.setAttribute("class", "ebBtn ebBtn_large");
//		addStateDirOutput.setAttribute("type",  "submit");
//		addStateDirOutput.setAttribute("value", Add New Direct State Output Mapping);
//		addStateDirOutput.innerHTML = addStateDirOutput.getAttribute("value");
		addStateDirOutput.onclick = function(){return editPolicyForm_State_addStateDirectOutput(dir_outputstable_body, false, statename, state, null, null, null, stateNextStateOptions, events);};
	}
	...
	logic_outputstable_head_tr.appendChild(document.createElement("th")); //empty, for delete button
	...
	if(state && state.stateFinalizerLogicMap && $.isArray(state.stateFinalizerLogicMap.entry)){
		for (var p = 0; p < state.stateFinalizerLogicMap.entry.length; p++) {
			...
			editPolicyForm_State_addStateLogicOutput(logic_outputstable_body, (createEditOrView == "VIEW"), statename, state, outputName,  logic, flavour);
		}
	}
...
//		addStateLogicOutput.setAttribute("id",    "editPolicyFormLogicOutputsAddOutput"+"_"+statename);
//		addStateLogicOutput.setAttribute("class", "ebBtn ebBtn_large");
//		addStateLogicOutput.setAttribute("type",  "submit");
//		addStateLogicOutput.setAttribute("value", "Add New Logic-based State Output Mapping / Finalizer");
//		addStateLogicOutput.innerHTML = addStateLogicOutput.getAttribute("value");
		addStateLogicOutput.onclick = function(){return editPolicyForm_State_addStateLogicOutput(logic_outputstable_body, false, statename, state, null, null, null);};
	}
	return retDiv;
}
function editPolicyForm_State_addStateLogicOutput(parentTBody, disabled, statename, state, outputName, logic, flavour){
	...
	var textarea = showHideTextarea("editEventFormfinalizerLogicInput"+"_"+statename+"_"+random_suffix, logic, false, !edit_readOnly, false);
	...
	finalizerlogicTypeLabel.setAttribute("for", "editPolicyFormFinalizerLogicTypeInput"+"_"+statename+"_"+random_suffix);
	...
	finalizerlogicTypeInput.setAttribute("id",          "editPolicyFormFinalizerLogicTypeInput"+"_"+statename+"_"+random_suffix);
	finalizerlogicTypeInput.setAttribute("type",        "text");
	finalizerlogicTypeInput.setAttribute("name",    	"editPolicyFormFinalizerLogicTypeInput"+"_"+statename+"_"+random_suffix);
...
function editPolicyForm_State_addStateDirectOutput(parentTBody, disabled, stateName, state, outputName,  nextState, outEvent, stateNextStateOptions, events){
	var random_suffix = formUtils_generateUUID();
	var outputTR =  parentTBody.insertRow(parentTBody.rows.length-1);
	...
	var nextstateSelectDiv = dropdownList("editPolicyFormOutputNextStateValue"+"_"+stateName+"_"+random_suffix, 
			stateNextStateOptions, nextState, disabled, null, function(){return editPolicyForm_getNextStateOptions();});
...
function editPolicyForm_State_addPolicyContext(parentTBody, disabled, stateName, contextName, contextreference, contextAlbums) {
	var random_suffix = formUtils_generateUUID();
	...
	var selectDiv = dropdownList("editPolicyFormContextValue"+"_"+stateName+"_"+random_suffix, 
			contextAlbums, contextreference, disabled, null);
	valueTD.appendChild(selectDiv);
}
	
function editPolicyForm_State_addPolicyTask(parentTBody, disabled, isdefault, state, stateName, taskreference, taskSelected, taskOptions) {
	var random_suffix = formUtils_generateUUID();
	...
	defaulttaskInput.setAttribute("name",        "editPolicyFormTaskIsDefault"+"_"+stateName); //group name
	...
	var selectDiv = dropdownList("editPolicyFormTaskValue"+"_"+stateName+"_"+random_suffix, 
			taskOptions, taskSelected, disabled, null);
	valueTD.appendChild(selectDiv);
	//output type 
	...
	outputTypeDirectLabel.setAttribute("for",		"editPolicyFormTaskOutputType"+"_"+stateName+"_"+random_suffix+"_DIRECT");
	...
	outputTypeDirectInput.setAttribute("id",		"editPolicyFormTaskOutputType"+"_"+stateName+"_"+random_suffix+"_DIRECT");
	outputTypeDirectInput.setAttribute("value",		"DIRECT");
	outputTypeDirectInput.setAttribute("type",		"radio");
	outputTypeDirectInput.setAttribute("name",		"editPolicyFormTaskOutputType"+"_"+stateName+"_"+random_suffix); //group name
	...
	outputTypeLogicLabel.setAttribute("for",		"editPolicyFormTaskOutputType"+"_"+stateName+"_"+random_suffix+"_LOGIC");
	...
	outputTypeLogicInput.setAttribute("id",			"editPolicyFormTaskOutputType"+"_"+stateName+"_"+random_suffix+"_LOGIC");
	outputTypeLogicInput.setAttribute("value",		"LOGIC");
	outputTypeLogicInput.setAttribute("type",		"radio");
	outputTypeLogicInput.setAttribute("name",		"editPolicyFormTaskOutputType"+"_"+stateName+"_"+random_suffix); //group name
	...
	if(taskreference != null && taskreference.output != null && taskreference.output.localName != null && taskreference.outputType != null && taskreference.outputType.toUpperCase() == "DIRECT"){
		dir_outputselected = {"name":taskreference.output.localName,"displaytext":taskreference.output.localName};
	}
	else if(taskreference != null && taskreference.output != null && taskreference.output.localName != null && taskreference.outputType != null && taskreference.outputType.toUpperCase() == "LOGIC"){
		logic_outputselected = {"name":taskreference.output.localName,"displaytext":taskreference.output.localName};
	}
	...
	if(state != null && state.stateOutputs  != null && $.isArray(state.stateOutputs.entry)){
		for (var p = 0; p < state.stateOutputs.entry.length; p++) {
			...
			dir_outputOptions.push({"name":outputEntry ,"displaytext":outputEntry});
		}
	}
	...
	if(state != null && state.stateFinalizerLogicMap  != null && $.isArray(state.stateFinalizerLogicMap.entry)){
		for (var p = 0; p < state.stateFinalizerLogicMap.entry.length; p++) {
			...
			logic_outputOptions.push({"name":outputEntry ,"displaytext":outputEntry});
		}
	}
	var dir_selectDiv = document.createElement("div");
	dir_selectDiv.appendChild( new dropdownList("editPolicyFormTaskDirectOutputSelection"+"_"+stateName+"_"+random_suffix, 
									dir_outputOptions, dir_outputselected, disabled, null, function(){return editPolicyForm_State_getDirectOutputMappingOptions(stateName);}));
	outputSelectionTD.appendChild(dir_selectDiv);
	var logic_selectDiv = document.createElement("div");
	logic_selectDiv.appendChild( dropdownList("editPolicyFormTaskLogicOutputSelection"+"_"+stateName+"_"+random_suffix, 
									logic_outputOptions, logic_outputselected, disabled, null, function(){return editPolicyForm_State_getLogicOutputMappingOptions(stateName);}));
	...
	outputTypeDirectInput.onclick = function() { if(this.checked){dir_selectDiv.style.display = "inline";logic_selectDiv.style.display = "none";}else{dir_selectDiv.style.display = "none" ;logic_selectDiv.style.display = "inline";}};
	outputTypeLogicInput.onclick =  function() { if(this.checked){dir_selectDiv.style.display = "none";logic_selectDiv.style.display = "inline";}else{dir_selectDiv.style.display = "inline" ;logic_selectDiv.style.display = "none";}};
}
...
function editPolicyForm_State_getLogicOutputMappingOptions(statename) {
	...
	if(finalizerstablerows && finalizerstablerows.length > 2){ //has head so just ignore (2) top row and bottom row
		for(var i = 1 ; i < finalizerstablerows.length-1 ; i++ ){
			...
			if(finalizerTR && finalizerTR.getAttribute("finalizer_id")){
				...
				var finalizerlocalname = document.getElementById("editPolicyFormLogicOutputNameValue_"+statename+"_"+finalizer_id).value;
				if(finalizerlocalname != null && finalizerlocalname != ""){
					outputoptions.push({"name":finalizerlocalname, "displaytext":finalizerlocalname});
				}
			}
		}
	}
...
function editPolicyForm_State_getDirectOutputMappingOptions(statename) {
	...
	if(outputstablerows && outputstablerows.length > 2){ //has head so just ignore (2) top row and bottom row
		for(var i = 1 ; i < outputstablerows.length-1 ; i++ ){
			...
			if(outputTR && outputTR.getAttribute("output_id")){
				...
				var outputlocalname = document.getElementById("editPolicyFormDirectOutputNameValue_"+statename+"_"+output_id).value;
				if(outputlocalname != null && outputlocalname != ""){
					outputoptions.push({"name":outputlocalname, "displaytext":outputlocalname});
				}
			}
		}
	}
...
function editPolicyForm_State_getStateBean(statename) {
	...
	if(triggervalue != null && triggervalue.event != null){
		ret.trigger = {"name" : triggervalue.event.key.name, "version" : triggervalue.event.key.version}; 
	}
	//context
	var statebean_context = new Array();
	var contextstablerows = document.getElementById("editPolicyFormContextsTable_"+statename).rows;
	if(contextstablerows && contextstablerows.length > 1){ //no head so just ignore (1) bottom row
		for(var i = 0 ; i < contextstablerows.length-1 ; i++ ){
			...
			if(contextTR && contextTR.getAttribute("context_id")){
				...
				var contextvalue = document.getElementById("editPolicyFormContextValue_"+statename+"_"+context_id+"_dropdownList").selectedOption;
				if(contextvalue != null && contextvalue.album != null){
					statebean_context.push({"name" : contextvalue.album.key.name, "version" : contextvalue.album.key.version});
				}
			}
		}
	}
	...
	if(outputstablerows && outputstablerows.length > 2){ //has head so just ignore (2) top row and bottom row
		for(var i = 1 ; i < outputstablerows.length-1 ; i++ ){
			...
			if(outputTR && outputTR.getAttribute("output_id")){
				...
				var nextstatevalue = document.getElementById("editPolicyFormOutputNextStateValue_"+statename+"_"+output_id+"_dropdownList").selectedOption;
				var nextstatename;
				if(nextstatevalue == null){
					console.error("An option must be selected for Next State for Direct Output Mapping "+outputlocalname+" for state "+statename);
					alert("An option must be selected for Next State for Direct Output Mapping "+outputlocalname+" for state "+statename);
					return null;
				}
				...
				var nexteventvalue = document.getElementById("editPolicyFormOutputEventValue_"+statename+"_"+output_id+"_dropdownList").selectedOption;
				if(nexteventvalue == null || nexteventvalue.event == null || nexteventvalue.event.key == null ){
					console.error("No Output Event selected for Direct Output Mapping "+outputlocalname+" for state "+statename);
					alert("No Output Event selected for Direct Output Mapping "+outputlocalname+" for state "+statename);
					return null;
				}
				var nextevent = {"name":nexteventvalue.event.key.name, "version":nexteventvalue.event.key.version};
				statebean_outputs[outputlocalname] = {"event":nextevent, "nextState":nextstatename};
			}
		}
	}
	...
	if(finalizerstablerows && finalizerstablerows.length > 2){ //has head so just ignore (2) top row and bottom row
		for(var i = 1 ; i < finalizerstablerows.length-1 ; i++ ){
			...
			if(finalizerTR && finalizerTR.getAttribute("finalizer_id")){
				...
				var finalizerlocalname = document.getElementById("editPolicyFormLogicOutputNameValue_"+statename+"_"+finalizer_id).value;
				if(finalizerlocalname == null || finalizerlocalname == ""){
					console.error("No Local Name entered for Logic-based Output Mapping #"+i+" for state "+statename);
					alert("No Local Name entered for Logic-based Output Mapping #"+i+" for state "+statename);
					return null;
				}

				var finalizerlogicvalue = document.getElementById("editEventFormfinalizerLogicInput_"+statename+"_"+finalizer_id+"_textarea").value;
				if(finalizerlogicvalue == null || finalizerlogicvalue == ""){
					console.error("No Logic is specified for Logic-based Output Mapping "+finalizerlocalname+" for state "+statename);
					alert("No Logic is specified for Logic-based Output Mapping "+finalizerlocalname+" for state "+statename);
					return null;
				}
				
				var finalizerlogictypevalue = document.getElementById("editPolicyFormFinalizerLogicTypeInput_"+statename+"_"+finalizer_id).value;
				if(finalizerlogictypevalue == null || finalizerlogictypevalue == ""){
					console.error("No Logic Type is specified for Logic-based Output Mapping "+finalizerlocalname+" for state "+statename);
					alert("No Logic Type is specified for Logic-based Output Mapping "+finalizerlocalname+" for state "+statename);
					return null;
				}
				statebean_finalizers[finalizerlocalname] = {"logic":finalizerlogicvalue, "logicFlavour":finalizerlogictypevalue};
			}
		}
	}
	...
	if(taskstablerows == null || taskstablerows.length <= 2){
		...
		for(var i = 1 ; i < taskstablerows.length-1 ; i++ ){
			...
			if(taskTR && taskTR.getAttribute("task_id")){
				...
				var tasklocalname = document.getElementById("editPolicyFormTaskLocalNameValue_"+statename+"_"+task_id).value;
				if(tasklocalname == ""){
				...
				if(statebean_tasks[tasklocalname] != null){
					console.error("There cannot be more than one task called "+tasklocalname+" for state "+statename);
					alert("There cannot be more than one task called "+tasklocalname+" for state "+statename);
					return null;
				}
				var taskvalue = document.getElementById("editPolicyFormTaskValue_"+statename+"_"+task_id+"_dropdownList").selectedOption;
				...
				var r = document.querySelector('input[name="editPolicyFormTaskOutputType_'+statename+'_'+task_id+'"]:checked');
				if(r == null){
					console.error("No Output Mapping type selected for task "+tasklocalname+" for state "+statename);
				...
				if(outputtype == "DIRECT"){
					var diroutput = document.getElementById("editPolicyFormTaskDirectOutputSelection_"+statename+"_"+task_id+"_dropdownList").selectedOption;
					if(diroutput == null){//} || diroutput.output == null){
						console.error("No DIRECT Output Mapping selected for task "+tasklocalname+" for state "+statename);
					...
					var logoutput = document.getElementById("editPolicyFormTaskLogicOutputSelection_"+statename+"_"+task_id+"_dropdownList").selectedOption;
					if(logoutput == null || logoutput.name == null){
						console.error("No LOGIC Output Mapping selected for task "+tasklocalname+" for state "+statename);
						alert("No LOGIC Output Mapping selected for task "+tasklocalname+" for state "+statename);
					}
					outputname = logoutput.name; 
				}
				else{
					console.error("Unknown Output Mapping type ('"+outputtype+"') selected for task "+tasklocalname+" for state "+statename);
					alert("Unknown Output Mapping type ('"+outputtype+"') selected for task "+tasklocalname+" for state "+statename);
					return null;
				}
				statebean_tasks[tasklocalname] = {"task":{"name":task.key.name,"version":task.key.version},"outputType":outputtype,"outputName":outputname};
				
				var r2 = document.getElementById("editPolicyFormTaskIsDefault_"+statename+"_"+task_id);
				if(taskstablerows.length <= 3 || (r2 != null && r2.checked == true)){ //default is checked or there is only one task
					statebean_defaultTask = {"name":task.key.name,"version":task.key.version}; 
				}
	...
	if(tsl == null || tsl == "" || tsl_type == null || tsl_type == ""){
		if(statebean_tasks != null && taskstablerows.length > 3) { //there is more than 1 task
			console.error("State "+statename+" has more than one task reference so Task Selection Logic and Task Selection Logic type must be specified");
			alert("State "+statename+" has more than one task reference so Task Selection Logic and Task Selection Logic type must be specified");
			return null;
		}
	}
		...
		ret["taskSelectionLogic"] = {"logic":tsl, "logicFlavour":tsl_type};
	}
