...
function editContextSchemaForm_activate(parent, operation, contextSchema) {
	...
	if (createEditOrView == "CREATE"){
		...
		if 	(!contextSchema)	{
			console.warn("Invalid value (\""+contextSchema+"\") passed as a value for \"contextSchema\" for ContextSchemaForm form.");
		}
		else{
			if	(!contextSchema.key || !contextSchema.key.name || contextSchema.key.name == "") {
				console.warn("Invalid value (\""+contextSchema.key.name+"\") passed as a value for \"name\" for ContextSchemaForm form.");
			} 
			if	(!contextSchema.key || !contextSchema.key.version || contextSchema.key.version == "") {
				console.warn("Invalid value (\""+contextSchema.key.version+"\") passed as a value for \"version\" for ContextSchemaForm form.");
			} 
			if	(!contextSchema.uuid || contextSchema.uuid == "") {
				console.warn("Invalid value (\""+contextSchema.uuid+"\") passed as a value for \"uuid\" for ContextSchemaForm form.");
			} 
			if (createEditOrView == "VIEW" ){
				if	(!contextSchema.description ) {
					console.warn("Invalid value (\""+contextSchema.description+"\") passed as a value for \"description\" for ContextSchemaForm form.");
				} 
				if	(!contextSchema.schemaFlavour || contextSchema.schemaFlavour == "") {
					console.warn("Invalid value (\""+contextSchema.schemaFlavour+"\") passed as a value for \"schemaFlavour\" for ContextSchemaForm form.");
				} 
				if	(!contextSchema.schemaDefinition || contextSchema.schemaDefinition == "") {
					console.warn("Invalid value (\""+contextSchema.schemaDefinition+"\") passed as a value for \"schemaDefinition\" for ContextSchemaForm form.");
				} 
			}
		}
	}
	else { 
		console.warn("Invalid operation (\""+operation+"\") specified for ContextSchemaForm form. Only \"Create\", \"Edit\" and \"View\" operations are supported");
	} 
...
function editContextSchemaForm_generateDescriptionPressed() {
	document.getElementById("editContextSchemaFormDescriptionTextArea").value = formUtils_generateDescription(
			document.getElementById("editContextSchemaFormNameInput"   ).value,
			document.getElementById("editContextSchemaFormVersionInput").value,
			document.getElementById("editContextSchemaFormUuidInput"   ).value);
}
...
function editContextSchemaForm_submitPressed() {
	...
	if (!createEditOrView || createEditOrView == "" || (createEditOrView != "CREATE" && createEditOrView != "EDIT")){
		console.error("Invalid operation \""+createEditOrView+"\" passed to editContextSchemaForm_submitPressed function. Edit failed");
		apexUtils_removeElement("editContextSchemaFormDiv");
		contextSchemaTab_reset();
		return;
	}
