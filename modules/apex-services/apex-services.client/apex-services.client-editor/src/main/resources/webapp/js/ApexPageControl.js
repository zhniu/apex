...
function pageControl_modelMode(name, version, fileName) {
	...
	$(".modelTag").show("slide", { direction: "left" }, 200);
	
	$("#mainTabs").tabs({classes: {
...
function pageControl_noModelMode() {
	$(".modelTag").hide("slide", { direction: "left" }, 200);
...
function pageControl_successStatus(data) {
	...
	if (data.OK) {
		...
		if(data.content.indexOf(sessionDoesNotExistStringStart) !== -1 && data.content.indexOf(sessionDoesNotExistStringEnd) !== -1 ) {
			clearLocalStorage();
			location.reload();
		}
...
function pageControl_restError(requestURL, jqXHR, textStatus, errorThrown) {
	...
	$("#statusMessageTable").empty();		    
	$("#statusMessageTable").append("<tr><td>request \"" + requestURL + "\" returned  \"" + textStatus + "\" " + jqXHR.status + " \"" + errorThrown + "\"</td></tr>");
