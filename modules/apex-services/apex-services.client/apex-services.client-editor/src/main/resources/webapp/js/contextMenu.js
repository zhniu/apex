...
//Hack: need to maintain each passed parameter in a way  it can be referenced by the onContextMenu handler function for 
//each iteration inside a loop, after the passed parameter goes out of scope and changes. 
...
function rightClickMenu_scopePreserver(parent, type, name, version) {
	return function(clickevent){rightClickMenu(clickevent, parent, type, name, version);}
}
...
function rightClickMenu(event, parent, type, name, version){
	...
	rclickdiv.addEventListener("contextmenu", function(e){e.preventDefault();}, false);
	...
	li1.onclick = function(){return rightClickMenuCreate(parent,type);};
	...
	if(name){
		...
		li4.onclick = function(){return rightClickMenuView(parent, type, name, version);};
		li4.innerHTML = "View <type>"+type+"</type> <value>"+value+"<value>";			 	
		ul.appendChild(li4); 
		
		var li2 = document.createElement("div");
		li2.setAttribute("class", "ebComponentList-item")
		li2.onclick = function(){return rightClickMenuEdit(parent, type, name, version);};
		...
		li3.onclick = function(){return rightClickMenuDelete(parent, type, name, version);};
	...
	// rclickdiv.setAttribute("class", "contextmenu_show"); ewatkmi: swapped with ebContextMenu
	rclickdiv.setAttribute("class", "ebContextMenu-body ebContextMenu-body_corner_default ebContextMenu-body_visible_true");
