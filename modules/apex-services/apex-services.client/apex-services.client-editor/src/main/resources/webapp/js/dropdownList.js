...
function dropdownList_option_select_scopePreserver(optionDiv, selectedOption, onselect){
	return function(event){dropdownList_option_select(optionDiv, selectedOption, onselect);}
}
function dropdownList_filter_scopePreserver(optionDiv, optionUl) {
	return function(event){dropdownList_filter(optionDiv, optionUl);}
}
...
function dropdownList_display_click(divname, options, selected, disabled, getoptions) {
	if(! document.getElementById(divname+"_options").classList.contains("dropdownList_show") ){ 
		...
		document.getElementById(divname+"_options").classList.add("dropdownList_show", "dropdownList_display_clicked");
	}
	else{
		document.getElementById(divname+"_options").classList.remove("dropdownList_show", "dropdownList_display_clicked");
	}
}
...
function dropdownList_filter(optionDiv, optionUl) {
	...
	var lis = ul.querySelectorAll("#"+ul.id+" > li");  //get li direct child elements 
 ...
 * 
 * Each dropdownList must have a unique "id_prefix" value as a unique identifier prefix
 * "options" is an array of objects for options, where each object has a field called name. options[i].displaytext is a displayed text for the option
 * "selected" is one of the options contained in list in the "options" parameter. "selected" must contain at least one field: selected.displaytext, where the value of selected.displaytext should be the same as options[i].displaytext for one of the values in options.    
 * "disabled" is a boolean, whether the drop down is enabled or disabled
 * "onselect" is a function that is called when an option is selected, with a parameter that is the selected option from the passed "options" 
 * "getoptions" is a function that is called to dynamically retrieve options. It is called when the drop-down list is checked. If this is set the "options" parameter is ignored. 
 * 
 * Returns a DIV representing the dropdown list.
 * The returned DIV will have a new field "selectedOption" containing one of the options in the "options" array, or null if no option is selected/
 * The returned DIV will have an id with value "id_prefix+'_dropdownList'", which should be unique.      
 */
...
function dropdownList_ChangeOptions(dropdownListDIV, options, selected, disabled, getoptions){
	...
	if(options == null && getoptions == null){
		console.error("Cannot provision dropdown list "+divname+" DIV because it has no options and no function to get options");
		return null;
	}
	if(disabled && !selected){
		console.warn("Provisioning dropdown list "+divname+" that is disabled, but there is no value selected!")
	}
	var subdivs = retdiv.querySelectorAll("#"+divname+" > div, button"); //get direct children or retdiv that are divs  
	...
	if(disabled){
		...
		var onClickFunction = function(event){dropdownList_display_click(divname, options, selected, disabled, getoptions);};
