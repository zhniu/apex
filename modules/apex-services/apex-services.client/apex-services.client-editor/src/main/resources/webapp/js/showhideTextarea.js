...
function showHideTextarea_display_hide(showHideDivprefix) {
	...
	if(sh["_clickable"] == true){
		sh.onclick = function(event){showHideTextarea_display_show(showHideDivprefix);};
	}
}
function showHideTextarea_display_show(showHideDivprefix) {
	...
	if(sh["_clickable"] == true){
		sh.onclick = function(event){showHideTextarea_display_hide(showHideDivprefix);};
	}
}
...
/*
 * Create a hideable textarea, inside a div, with some text displayed in a clickable area to show the text area, and some other text displayed in the clickable area to hide the text area 
 * 
 * Each showHideTextarea must have a unique "id_prefix" vale as an identifier
 * "content" is the text to be put into the text area
 * "initialshow" is a boolean, if true the textarea will be shown initially, otherwise hidden
 * "editable" is a boolean, if true the textarea will be editable, otherwise hidden
 * "disabled" is a boolean, if true the textarea can be shown, otherwise it canot be shown
 * "showText" is the text that is shown to be clicked to show the text area
 * "hideText" is the text that is shown to be clicked to show the text area
 * 
 * Returns a div representing the hideable textarea, with id 'id_prefix'.
 * The returned div will a textarea called "id_prefix+'_textarea'", that can be queried to get the value of the textarea      
 */
...
function showHideTextarea(id_prefix, content, _initialshow, _editable, _disabled, _showText, _hideText) {
	...
	checkbox.change(function(event) {
		if(!shown) {
			createEditArea(textArea.getAttribute("id"), {is_editable: editable});
			shown = true;
		} else {
			$(parent).toggle("fast");			
		}
