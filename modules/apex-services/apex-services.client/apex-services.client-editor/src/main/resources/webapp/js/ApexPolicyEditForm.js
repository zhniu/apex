...
function editPolicyForm_editPolicy_inner (formParent, policy, viewOrEdit){
	...
	ajax_get(requestURL, function(data2) {
		for (var i = 0; i < data2.messages.message.length; i++) {
			...
			contextSchemas.push({	"name":contextSchema.key.name,  "version":contextSchema.key.version, 
		...
		ajax_get(requestURL, function(data3) {
			for (var j = 0; j < data3.messages.message.length; j++) {
				...
				tasks.push({	"name":task.key.name,  "version":task.key.version, 
			...
			ajax_get(requestURL, function(data4) {
				for (var k = 0; k < data4.messages.message.length; k++) {
					...
					albums.push({	"name":album.key.name,  "version":album.key.version, 
				...
				ajax_get(requestURL, function(data5) {
					for (var m = 0; m < data5.messages.message.length; m++) {
						...
						events.push({	"name":event.key.name,  "version":event.key.version, 
...
function editPolicyForm_activate(parent, operation, policy,  tasks, events, contextAlbums, contextItemSchemas) {
	...
	if (createEditOrView == "CREATE"){
		...
		if 	(!policy)	{
			...
			if	(!policy.policyKey || !policy.policyKey.name || policy.policyKey.name == "") {
				console.warn("Invalid value (\""+policy.policyKey.name+"\") passed as a value for \"name\" for PolicyForm form.");
			} 
			if	(!policy.policyKey || !policy.policyKey.version || policy.policyKey.version == "") {
				console.warn("Invalid value (\""+policy.policyKey.version+"\") passed as a value for \"version\" for PolicyForm form.");
			} 
			if	(!policy.uuid || policy.uuid == "") {
				console.warn("Invalid value (\""+policy.uuid+"\") passed as a value for \"uuid\" for PolicyForm form.");
			} 
		}
	}
	else { 
		console.warn("Invalid operation (\""+operation+"\") specified for PolicyForm form. Only \"Create\", \"Edit\" and \"View\" operations are supported");
	} 
	...
	if( policy != null && policy.state != null){
		for (i = 0 ; i < policy.state.entry.length ; i++ ){
			if(policy.state.entry[i] != null && policy.state.entry[i].key != null){
				...
				firststateoptions.push({"name":statename, "displaytext":statename});
			}
		}
	}
	if( policy != null && policy.firstState != null && policy.firstState != "" ){
		firststateselected = {"name":policy.firstState, "displaytext":policy.firstState}
	}
	var firstStateSelectDiv = dropdownList("editEventFormSelectFirstState", 
								firststateoptions, firststateselected, 
								(createEditOrView != "CREATE" && createEditOrView != "EDIT" ), 
								function(){return editPolicyForm_updateTriggerEventOptions(events);},
								function(){return editPolicyForm_getStateOptions();}
						);
		...
		for (i = 0 ; i < policy.state.entry.length ; i++ ){
			if(policy.state.entry[i] != null && policy.state.entry[i].key != null){
				...
				if(statename != null && statename == policy.firstState){
					triggerevent =  {"name":state.trigger.name,"version":state.trigger.version,"displaytext":state.trigger.name+":"+state.trigger.version};
				}
			}
		}
	}
	// var triggerSelectDiv = dropdownList("editEventFormSelectTrigger", events, triggerevent, true, null); // ewatkmi: replaced dropdown with label
	...
	if (createEditOrView == "CREATE" || createEditOrView == "EDIT" ){
		...
		addState.onclick = function(){return editPolicyForm_addNewState(statesUL, createEditOrView, policy, tasks, events, contextAlbums, contextItemSchemas);};
...
function editPolicyForm_addNewState(statesUL, createEditOrView, policy, tasks, events, contextAlbums, contextItemSchemas){
	...
	if(statename == null || statename  == "" ){
		...
		alert("Please enter a valid value for the name of the new state\""+paramname+"\". Values \"NULL\" and \"None\" are not allowed");
		document.getElementById("editEventFormNewStateInput").focus();
		return false;
	}
	else{
	...
	if( policy && policy.state ){
		for (i = 0 ; i < policy.state.entry.length ; i++ ){
			if(statename.toUpperCase() == policy.state.entry[i].key.toUpperCase()){
				alert("Policy "+policy.policyKey.name+":"+policy.policyKey.version+" already contains a state called \""+statename+"\".");
				document.getElementById("editEventFormNewStateInput").focus();
				return false;
			} 
		}
	}
	var stateLI = editPolicyForm_addState(statename, null, createEditOrView, policy, tasks, events, contextAlbums, contextItemSchemas); 
...
function editPolicyForm_getStateOptions(){
	...
	var stateslis = document.getElementById("editEventFormStates").querySelectorAll("#editEventFormStates > li[stateName]");  //get li direct child elements with an attribute "stateName"
	for (var i = 0; i < stateslis.length; i++) {  
		if(stateslis != null && stateslis[i] != null && stateslis[i].getAttribute("stateName") != null){
			stateoptions.push({"name":stateslis[i].getAttribute("stateName"), "displaytext":stateslis[i].getAttribute("stateName")});
		}
	}
	return stateoptions;
}
...
function editPolicyForm_getNextStateOptions(){
	...
	stateoptions.push({"name":"NULL","displaytext":"None"}); 
	return stateoptions;
}
...
function editPolicyForm_addState(statename, state, createEditOrView, policy, tasks, events, contextAlbums, contextItemSchemas){
	...
	if(createEditOrView == "CREATE" || createEditOrView == "EDIT"){
		...
		deleteDiv.onclick = function(event){
			$(stateLI).hide("fast", function() {
				stateLI.parentNode.removeChild(stateLI);
				var firstState = document.getElementById("editEventFormSelectFirstState_dropdownList_display").innerHTML;
				var selected = (statename !== firstState) ? {name: firstState, displaytext: firstState} : undefined; 
				dropdownList_ChangeOptions(document.getElementById("editEventFormSelectFirstState_dropdownList"), editPolicyForm_getStateOptions(), selected, false, editPolicyForm_getStateOptions);
	...
	var stateDiv = editPolicyForm_State_generateStateDiv(createEditOrView, policy, statename, state,  tasks, events, contextAlbums, contextItemSchemas);
	var showhideDIV = showHideElement("editEventFormStates_"+statename, stateDiv, true, "Show "+statename+" ...", "Hide "+statename+" ...", "showhide_div_show", "showhide_div_hide", "showhide_button_show", "showhide_button_hide");
...
function editPolicyForm_generateDescriptionPressed() {
	document.getElementById("editPolicyFormDescriptionTextArea").value = formUtils_generateDescription(
			document.getElementById("editPolicyFormNameInput"   ).value,
			document.getElementById("editPolicyFormVersionInput").value,
			document.getElementById("editPolicyFormUuidInput"   ).value);
}
...
function editPolicyForm_submitPressed() {
	...
	if (!createEditOrView || createEditOrView == "" || (createEditOrView != "CREATE" && createEditOrView != "EDIT")){
		console.error("Invalid operation \""+createEditOrView+"\" passed to editPolicyForm_submitPressed function. Edit failed");
		apexUtils_removeElement("editPolicyFormDiv");
		policyTab_reset();
		return;
	}
...
function editPolicyForm_getPolicyBean() {
	...
	var stateslis = document.getElementById("editEventFormStates").querySelectorAll("#editEventFormStates > li[stateName]");  //get li direct child elements with an attribute "stateName"
	for (var i = 0; i < stateslis.length; i++) { //ignore last li ... it has the new state button etc. 
		if(stateslis != null && stateslis[i] != null && stateslis[i].getAttribute("stateName") != null){
