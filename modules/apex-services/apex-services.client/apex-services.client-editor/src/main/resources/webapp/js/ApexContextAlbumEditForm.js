...
function editContextAlbumForm_activate(parent, operation, contextAlbum, contextSchemas) {
	...
	if (createEditOrView == "CREATE"){
		...
		if 	(!contextAlbum)	{
			console.warn("Invalid value (\""+contextAlbum+"\") passed as a value for \"contextAlbum\" for ContextAlbumForm form.");
		}
		else{
			if	(!contextAlbum.key || !contextAlbum.key.name || contextAlbum.key.name == "") {
				console.warn("Invalid value (\""+contextAlbum.key.name+"\") passed as a value for \"name\" for ContextAlbumForm form.");
			} 
			if	(!contextAlbum.key || !contextAlbum.key.version || contextAlbum.key.version == "") {
				console.warn("Invalid value (\""+contextAlbum.key.version+"\") passed as a value for \"version\" for ContextAlbumForm form.");
			} 
			if	(!contextAlbum.uuid || contextAlbum.uuid == "") {
				console.warn("Invalid value (\""+contextAlbum.uuid+"\") passed as a value for \"uuid\" for ContextAlbumForm form.");
			} 
			if (createEditOrView == "VIEW" ){
				if	(!contextAlbum.description ) {
					console.warn("Invalid value (\""+contextAlbum.description+"\") passed as a value for \"description\" for ContextAlbumForm form.");
				} 
				if	(!contextAlbum.scope || contextAlbum.scope == "") {
					console.warn("Invalid value (\""+contextAlbum.scope+"\") passed as a value for \"scope\" for ContextAlbumForm form.");
				} 
				if	(!contextAlbum.itemSchema || !contextAlbum.itemSchema.name || contextAlbum.itemSchema.name == "" || !contextAlbum.itemSchema.version || contextAlbum.itemSchema.version == "") {
					console.warn("Invalid value (\""+contextAlbum.itemSchema+"\") passed as a value for \"itemSchema\" for ContextAlbumForm form.");
				} 
				if	(!contextAlbum.isWritable || contextAlbum.isWritable == "") {
					console.warn("Invalid value (\""+contextAlbum.isWritable+"\") passed as a value for \"isWritable\" for ContextAlbumForm form.");
				} 
			}
		}
	}
	else { 
		console.warn("Invalid operation (\""+operation+"\") specified for ContextAlbumForm form. Only \"Create\", \"Edit\" and \"View\" operations are supported");
	} 
	...
	if (contextAlbum && contextAlbum.itemSchema != null && contextAlbum.itemSchema.name != null 
			&& contextAlbum.itemSchema.version != null 
			&& contextAlbum.itemSchema.name != "" && contextAlbum.itemSchema.version != "" ){
		selected = {	"name":contextAlbum.itemSchema.name, "version":contextAlbum.itemSchema.version,
...
function editContextAlbumForm_generateDescriptionPressed() {
	document.getElementById("editContextAlbumFormDescriptionTextArea").value = formUtils_generateDescription(
			document.getElementById("editContextAlbumFormNameInput"   ).value,
			document.getElementById("editContextAlbumFormVersionInput").value,
			document.getElementById("editContextAlbumFormUuidInput"   ).value);
}
...
function editContextAlbumForm_submitPressed() {
	...
	if (!createEditOrView || createEditOrView == "" || (createEditOrView != "CREATE" && createEditOrView != "EDIT")){
		console.error("Invalid operation \""+createEditOrView+"\" passed to editContextAlbumForm_submitPressed function. Edit failed");
		apexUtils_removeElement("editContextAlbumFormDiv");
		contextAlbumTab_reset();
		return;
	}
	...
	var itemschema = {"name":selectedschema.name,"version":selectedschema.version};
	
	var scope = $('#editContextAlbumFormAlbumScopeInput').val().toUpperCase();
	if(scope != "APPLICATION" && scope != "EXTERNAL" && scope != "GLOBAL"){
		var message = "Are you sure you want to set the scope ContextAlbum to \""+scope+"\"? Currently the only scope supported are: \"APPLICATION\", \"EXTERNAL\" and \"GLOBAL\"";
		if(! apexUtils_areYouSure(message)){
			return false;
		}
	}
	...
	if(readonly && scope != "EXTERNAL"){
		var message = "Are you sure you want to set the scope ContextAlbum to \""+scope+"\" while it is set as Read Only. It only makes sense to use readonly for \"EXTERNAL\" Context Albums. If this Album is readonly it cannot be set!";
		if(! apexUtils_areYouSure(message)){
			return false;
		}
	}
	if(!readonly && scope == "EXTERNAL"){
		var message = "Are you sure you want to set the scope ContextAlbum to \""+scope+"\" while it is not set as Read Only. It only makes sense to use readonly for \"EXTERNAL\" Context Albums since EXTERNAL context cannot be set in policies.";
		if(! apexUtils_areYouSure(message)){
			return false;
		}
	}
