function editTaskForm_createTask (formParent){
	...
	ajax_get(requestURL, function(data2) {
		for (var i = 0; i < data2.messages.message.length; i++) {
			...
			var dt = {"name":contextSchema.key.name,"version":contextSchema.key.version,"displaytext":contextSchema.key.name+":"+contextSchema.key.version,"contextSchema":contextSchema};
		...
		ajax_get(requestURL, function(data3) {
			for (var i = 0; i < data3.messages.message.length; i++) {
				...
				var ca = {"name":contextAlbum.key.name,"version":contextAlbum.key.version,"displaytext":contextAlbum.key.name+":"+contextAlbum.key.version,"contextAlbum":contextAlbum};
...
function editTaskForm_editTask_inner (formParent, name, version, viewOrEdit){
	...
	ajax_get(requestURL, function(data) {
		...
		ajax_get(requestURL, function(data2) {
			for (var i = 0; i < data2.messages.message.length; i++) {
				var contextSchema = JSON.parse(data2.messages.message[i]).apexContextSchema;
				contextSchemas.push({"name":contextSchema.key.name,"version":contextSchema.key.version,"displaytext":contextSchema.key.name+":"+contextSchema.key.version,"contextSchema":contextSchema});
			}
			//Get all contextAlbums too for task context album references
			var requestURL = restRootURL + "/ContextAlbum/Get?name=&version=";
			var contextAlbums = new Array();
...
function editTaskForm_activate(parent, operation, task, contextSchemas, contextAlbums) {
	...
	if (createEditOrView == "CREATE"){
		...
		if 	(!task)	{
			...
			if	(!task.key || !task.key.name || task.key.name == "") {
				console.warn("Invalid value (\""+task.key.name+"\") passed as a value for \"name\" for TaskForm form.");
			} 
			if	(!task.key || !task.key.version || task.key.version == "") {
				console.warn("Invalid value (\""+task.key.version+"\") passed as a value for \"version\" for TaskForm form.");
			} 
		...
		console.warn("Invalid operation (\""+operation+"\") specified for TaskForm form. Only \"Create\", \"Edit\" and \"View\" operations are supported");
	} 
	...
	inputfieldstable_head_tr.appendChild(document.createElement("th")); //empty, for delete button
	...
	if(task && task.inputFields && task.inputFields.entry){
		for (var p = 0; p < task.inputFields.entry.length; p++) {
			...
			editTaskForm_addTaskInputField(inputfieldstable_body, (createEditOrView == "VIEW"), inputfieldEntry.key, inputfieldEntry.value.optional, contextSchema, contextSchemas);
		}
	}
	...
	if (createEditOrView == "CREATE" || createEditOrView == "EDIT"){
		...
		addInputFieldInput.onclick = function(){editTaskForm_addTaskInputField(inputfieldstable_body, false,null,false,null,contextSchemas);};
	}
	...
	outputfieldstable_head_tr.appendChild(document.createElement("th")); //empty, for delete button
	...
	if(task && task.outputFields && task.outputFields.entry){
		for (var p = 0; p < task.outputFields.entry.length; p++) {
			...
			contextSchema["displaytext"] = contextSchema.name+":"+contextSchema.version;
			editTaskForm_addTaskOutputField(outputfieldstable_body, (createEditOrView == "VIEW"), outputfieldEntry.key, outputfieldEntry.value.optional, contextSchema, contextSchemas);
		}
	}
	...
	if (createEditOrView == "CREATE" || createEditOrView == "EDIT"){
		...
		addOutputFieldInput.onclick = function(){editTaskForm_addTaskOutputField(outputfieldstable_body, false,null,false,null,contextSchemas);};
	}
	...
	paramstable_head_tr.appendChild(document.createElement("th")); //empty, for delete button
	...
	if (createEditOrView == "CREATE" || createEditOrView == "EDIT"){
		...
		addParamInput.onclick = function(){editTaskForm_addTaskParameter(paramstable_body, false, null, null);};
	}
	...
	if(task && task.contextAlbumReference && $.isArray(task.contextAlbumReference)){
		for (var p = 0; p < task.contextAlbumReference.length; p++) {
			...
			editTaskForm_addTaskContext(contextstable_body, (createEditOrView == "VIEW"), contextName, ce, contextAlbums);
		}
	}
	...
	if (createEditOrView == "CREATE" || createEditOrView == "EDIT"){
		...
		addContextInput.onclick = function(){editTaskForm_addTaskContext(contextstable_body, false, null, null, contextAlbums);};
	}
...
function editTaskForm_addTaskInputField(parentTBody, disabled, name, optional, contextSchema, contextSchemas) {
	...
	var selectDiv = dropdownList("editTaskFormInputFieldContextSchema"+"_"+random_suffix, 
			contextSchemas, contextSchema, disabled, null)
...
function editTaskForm_addTaskOutputField(parentTBody, disabled, name, optional, contextSchema, contextSchemas) {
	...
	var selectDiv = dropdownList("editTaskFormOutputFieldContextSchema"+"_"+random_suffix, 
			contextSchemas, contextSchema, disabled, null)
	contextSchemaTD.appendChild(selectDiv);
...
function editTaskForm_addTaskContext(parentTBody, disabled, name, albumreference, contextAlbums) {
	...
	var selectDiv = dropdownList("editTaskFormContextValue"+"_"+random_suffix, 
			contextAlbums, albumreference, disabled, null);
...
function editTaskForm_generateDescriptionPressed() {
	document.getElementById("editTaskFormDescriptionTextArea").value = formUtils_generateDescription(
			document.getElementById("editTaskFormNameInput"   ).value,
			document.getElementById("editTaskFormVersionInput").value,
...
function editTaskForm_submitPressed() {
	...
	if (!createEditOrView || createEditOrView == "" || (createEditOrView != "CREATE" && createEditOrView != "EDIT")){
		console.error("Invalid operation \""+createEditOrView+"\" passed to editTaskForm_submitPressed function. Edit failed");
		apexUtils_removeElement("editTaskFormDiv");
		taskTab_reset();
		return;
	}
	...
	if(inputfieldstablerows && inputfieldstablerows.length >2){
		...
		for(var i = 1 ; i < inputfieldstablerows.length-1 ; i++ ){
			...
			if(inputfieldTR && inputfieldTR.getAttribute("inputfield_id")){
				...
				var inputfield_optional = document.getElementById("editTaskFormInputFieldOptional"+"_"+inputfield_id).checked;
				var inputfield_dt = document.getElementById("editTaskFormInputFieldContextSchema"+"_"+inputfield_id+"_dropdownList").selectedOption;
				...
				if(inputfield_dt == null){
					alert("Task \""+name+"\" has no selected Context Item Schema for the Input Field called \""+inputfieldname+"\"");
					return false;
				}
				var inputfield_dt_name =   inputfield_dt.name;
				var inputfield_dt_version =  inputfield_dt.version;
				taskbean_inputfields[inputfieldname] = {"localName":inputfieldname,"name":inputfield_dt_name,"version":inputfield_dt_version,"optional": inputfield_optional};
			}
		}
	}
	...
	if(outputfieldstablerows && outputfieldstablerows.length >2){
		...
		for(var i = 1 ; i < outputfieldstablerows.length-1 ; i++ ){
			...
			if(outputfieldTR && outputfieldTR.getAttribute("outputfield_id")){
				...
				var outputfield_optional = document.getElementById("editTaskFormOutputFieldOptional"+"_"+outputfield_id).checked;
				var outputfield_dt = document.getElementById("editTaskFormOutputFieldContextSchema"+"_"+outputfield_id+"_dropdownList").selectedOption;
				...
				if(outputfield_dt == null){
					alert("Task \""+name+"\" has no selected Context Item Schema for the Output Field called \""+outputfieldname+"\"");
					return false;
				}
				var outputfield_dt_name =     outputfield_dt.name;
				var outputfield_dt_version =  outputfield_dt.version;
				taskbean_outputfields[outputfieldname] = {"localName":outputfieldname,"name":outputfield_dt_name,"version":outputfield_dt_version,"optional": outputfield_optional};
			}
		}
	}
	...
	var tasklogic = {"logic":logicfield, "logicFlavour":logictype};
	...
	if(paramstablerows && paramstablerows.length >2){
		...
		for(var i = 1 ; i < paramstablerows.length-1 ; i++ ){
			...
			if(paramTR && paramTR.getAttribute("param_id")){
				...
				taskbean_parameters[paramname] = {"parameterName":paramname, "defaultValue":paramvalue};
			}
		}
	}
	...
	if(contextstablerows && contextstablerows.length >2){
		...
		for(var i = 1 ; i < contextstablerows.length-1 ; i++ ){
			...
			if(contextTR && contextTR.getAttribute("context_id")){
				...
				var contextalbumvalue = document.getElementById("editTaskFormContextValue"+"_"+context_id+"_dropdownList").selectedOption;
				...
				for(var j = 0 ; j < taskbean_context.length ; j++ ){
					if(taskbean_context[j] != null && taskbean_context[j].name == contextalbumvalue.name && taskbean_context[j].version == contextalbumvalue.version ){
						alert("Task \""+name+"\" references Context Album \""+contextalbumname+"\" more than once");
						return false;
					}
				}
				taskbean_context.push({"name":contextalbumvalue.name, "version":contextalbumvalue.version});
			}
		}
	}
