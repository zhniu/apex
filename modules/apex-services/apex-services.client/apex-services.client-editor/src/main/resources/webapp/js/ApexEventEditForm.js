...
function editEventForm_activate(parent, operation, event, contextSchemas) {
	...
	if (createEditOrView == "CREATE"){
		...
		if 	(!event)	{
			...
			if	(!event.key || !event.key.name || event.key.name == "") {
				console.warn("Invalid value (\""+event.key.name+"\") passed as a value for \"name\" for EventForm form.");
			} 
			if	(!event.key || !event.key.version || event.key.version == "") {
				console.warn("Invalid value (\""+event.key.version+"\") passed as a value for \"version\" for EventForm form.");
			} 
			if	(!event.uuid || event.uuid == "") {
				console.warn("Invalid value (\""+event.uuid+"\") passed as a value for \"uuid\" for EventForm form.");
			} 
		}
	}
	else { 
		console.warn("Invalid operation (\""+operation+"\") specified for EventForm form. Only \"Create\", \"Edit\" and \"View\" operations are supported");
	} 
	...
	if(event && event.parameter && event.parameter.entry){
		for (var p = 0; p < event.parameter.entry.length; p++) {
			...
			editEventForm_addEventParam(paramstable_body, (createEditOrView == "VIEW"), fieldEntry.key, optional, contextSchema, contextSchemas);
		}
	}
	...
	if (createEditOrView == "CREATE" || createEditOrView == "EDIT"){
		...
		addParamInput.onclick = function(){editEventForm_addEventParam(paramstable_body, false,null,false,null,contextSchemas);};
	}
...
function editEventForm_addEventParam(parentTBody, disabled, name, optional, contextSchema, contextSchemas) {
	...
	var selectDiv = dropdownList("editEventFormParamContextSchema"+"_"+random_suffix, 
...
function editEventForm_generateDescriptionPressed() {
	document.getElementById("editEventFormDescriptionTextArea").value = formUtils_generateDescription(
			document.getElementById("editEventFormNameInput"   ).value,
			document.getElementById("editEventFormVersionInput").value,
			document.getElementById("editEventFormUuidInput"   ).value);
}
...
function editEventForm_submitPressed() {
	...
	if (!createEditOrView || createEditOrView == "" || (createEditOrView != "CREATE" && createEditOrView != "EDIT")){
		console.error("Invalid operation \""+createEditOrView+"\" passed to editEventForm_submitPressed function. Edit failed");
		apexUtils_removeElement("editEventFormDiv");
		eventTab_reset();
		return;
	}
	...
	if(paramstablerows && paramstablerows.length >=2){
		...
		for(var i = 1 ; i < paramstablerows.length-1 ; i++ ){
			...
			if(paramTR && paramTR.getAttribute("param_id")){
				...
				var param_dt = document.getElementById("editEventFormParamContextSchema"+"_"+param_id+"_dropdownList").selectedOption;
				if(eventbean_params[paramname]){
					alert("Event \""+name+"\" contains more than one Parameter called \""+paramname+"\"");
					return false;
				}
				if(param_dt == null){
					alert("Event \""+name+"\" has no selected Context Item Schema for the Parameter called \""+paramname+"\"");
					return false; 
				}
				var param_dt_name =   param_dt.name;
				var param_dt_version =  param_dt.version;
				eventbean_params[paramname] = {"localName":paramname,"name":param_dt_name,"version":param_dt_version, "optional":paramoptional};
			}
		}
	}
