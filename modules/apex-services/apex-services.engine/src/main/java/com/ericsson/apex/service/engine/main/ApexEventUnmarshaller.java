...
public class ApexEventUnmarshaller implements ApexEventReceiver, Runnable {
	 ...
	 */
	public void connectMarshaler(EventHandlerPeeredMode peeredMode, final ApexEventMarshaller peeredMarshaller) {
		switch (peeredMode) {
		case SYNCHRONOUS:
			// To connect a synchronous unmarshaler and marshaler, we create a synchronous event cache on the consumer/producer pair
			new SynchronousEventCache(peeredMode, consumer, peeredMarshaller.getProducer(), consumerParameters.getPeerTimeout(EventHandlerPeeredMode.SYNCHRONOUS));
			return;
		...
		try {
			...
			for (ApexEvent apexEvent : apexEventList) {
				...
				if (consumerParameters.isPeeredMode(EventHandlerPeeredMode.SYNCHRONOUS)) {
					SynchronousEventCache synchronousEventCache = (SynchronousEventCache) consumer.getPeeredReference(EventHandlerPeeredMode.SYNCHRONOUS);
