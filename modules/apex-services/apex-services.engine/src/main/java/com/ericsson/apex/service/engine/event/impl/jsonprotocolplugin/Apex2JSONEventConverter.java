...
public class Apex2JSONEventConverter implements ApexEventProtocolConverter {
	 ...
	 * @throws ApexEventException thrown on unmarshaling exceptions
	 */
	private ApexEvent jsonObject2ApexEvent(final String eventName, final JsonObject jsonObject) throws ApexEventException {
	 ...
	 */
	private ApexEvent processApexEventHeader(final String eventName, final JsonObject jsonObject) throws ApexEventRuntimeException, ApexEventException {
		...
		if (eventName != null) {
			if (name != null && !eventName.equals(name)) {
				LOGGER.warn("The incoming event name \"" + name + "\" does not match the configured event name \"" + eventName + "\", using configured event name");
			}
			name = eventName;
		}
		else {
			if (name == null) {
				throw new ApexEventRuntimeException("event received without mandatory parameter \"name\" on configuration or on event");
			}
		}
