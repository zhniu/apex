...
/**
 * The Class Apex2JSONEventConverter converts {@link ApexEvent} instances to and from JSON string representations of Apex events.
 *
...
public class Apex2JSONEventConverter implements ApexEventProtocolConverter {
    ...
    public void init(final EventProtocolParameters parameters) {
        ...
        if (!(parameters instanceof JSONEventProtocolParameters)) {
            String errorMessage = "specified consumer properties are not applicable to the JSON event protocol";
    ...
    public List<ApexEvent> toApexEvent(final Object eventObject) throws ApexEventException {
        ...
        List<ApexEvent> eventList = new ArrayList<ApexEvent>();
        ...
        try {
            ...
            if (decodedJsonObject instanceof List) {
                ...
                List<Object> decodedJsonList = (List<Object>) decodedJsonObject;

                // Decode each of the list elements in sequence
                for (Object jsonListObject : decodedJsonList) {
                    if (jsonListObject instanceof String) {
                        eventList.add(jsonStringApexEvent((String) jsonListObject));
                    }
                    else if (jsonListObject instanceof JsonObject) {
                        eventList.add(jsonObject2ApexEvent((JsonObject) jsonListObject));
                    }
                    else {
                        throw new ApexEventException(
                                "incoming event (" + jsonEventString + ") is a JSON object array containing an invalid object " + jsonListObject);
                    }
                }
            }
            else {
                eventList.add(jsonStringApexEvent(jsonEventString));
            }
        }
        catch (final Exception e) {
            final String errorString = "Failed to unmarshal JSON event: " + e.getMessage() + ", event=" + jsonEventString;
        ...
        for (final AxField eventField : eventDefinition.getFields()) {
            ...
            if (!apexEvent.containsKey(fieldName)) {
                if (!eventField.getOptional()) {
                    String errorMessage = "error parsing " + eventDefinition.getID() + " event to Json. " + "Field \"" + fieldName
                            + "\" is missing, but is mandatory. Fields: " + apexEvent;
            ...
            final SchemaHelper fieldSchemaHelper = new SchemaHelperFactory().createSchemaHelper(eventField.getKey(), eventField.getSchema());
     ...
     */
    private ApexEvent jsonStringApexEvent(final String jsonEventString) throws ApexEventException {
        ...
        return jsonObject2ApexEvent(jsonObject);
    }
     ...
     * @throws ApexEventException thrown on unmarshaling exceptions
     */
    private ApexEvent jsonObject2ApexEvent(final JsonObject jsonObject) throws ApexEventException {
        // Process the mandatory Apex header
        final ApexEvent apexEvent = processApexEventHeader(jsonObject);
        ...
        for (final AxField eventField : eventDefinition.getFields()) {
            ...
            if (!hasJSONField(jsonObject, fieldName)) {
                if (!eventField.getOptional()) {
                    String errorMessage = "error parsing " + eventDefinition.getID() + " event from Json. " + "Field \"" + fieldName
                            + "\" is missing, but is mandatory.";
                    LOGGER.debug(errorMessage);
     ...
     */
    private ApexEvent processApexEventHeader(final JsonObject jsonObject) throws ApexEventRuntimeException, ApexEventException {
        // Get the event header fields
        // @formatter:off
        String name      = getJSONStringField(jsonObject, ApexEvent.NAME_HEADER_FIELD,      jsonPars.getNameAlias(),      ApexEvent.NAME_REGEXP,      true);
        ...
        // Now, find the event definition in the model service. If version is null, the newest event definition in the model service is used
        final AxEvent eventDefinition = ModelService.getModel(AxEvents.class).get(name, version);
        if (eventDefinition == null) {
            if (version == null) {
                throw new ApexEventRuntimeException("an event definition for an event named \"" + name + "\" not found in Apex model");
            }
            else {
                throw new ApexEventRuntimeException(
                        "an event definition for an event named \"" + name + "\" with version \"" + version + "\" not found in Apex model");
            }
        }
        ...
        if (namespace != null) {
            if (!namespace.equals(eventDefinition.getNameSpace())) {
                throw new ApexEventRuntimeException("namespace \"" + namespace + "\" on event \"" + name + "\" does not match namespace \""
     ...
     * @throws ApexEventRuntimeException the apex event runtime exception
     */
    private String getJSONStringField(final JsonObject jsonObject, final String fieldName, final String fieldAlias, final String fieldRE,
        ...
        try {
            ...
            throw new ApexEventRuntimeException(
                    "field \"" + fieldName + "\" with type \"" + jsonField.getClass().getCanonicalName() + "\" is not a string value");
        }
        ...
        if (!fieldValueString.matches(fieldRE)) {
            throw new ApexEventRuntimeException("field \"" + fieldName + "\" with value \"" + fieldValueString + "\" is invalid");
        }
     ...
     * @throws ApexEventRuntimeException the apex event runtime exception
     */
    private JsonElement getJSONField(final JsonObject jsonObject, final String fieldName, final String fieldAlias, final boolean mandatory)
