...
/**
 * The listener interface for receiving engDepMessage events. The class that is interested in processing a engDepMessage event implements this interface, and
 * the object created with that class is registered with a component using the component's <code>addEngDepMessageListener</code> method. When the engDepMessage
 * event occurs, that object's appropriate method is invoked.
 *
 * This class uses a queue to buffer incoming messages. When the listener is called, it places the incoming message on the queue. A thread runs which removes
 * the messages from the queue and forwards them to the Apex engine.
 *
...
public class EngDepMessageListener implements MessageListener<Message>, Runnable {
    ...
    /**
     * Instantiates a new EngDep message listener for listening for messages coming in from the Deployment client. The <code>apexService</code> is the Apex
     * service to send the messages onto.
     *
    ...
    /**
     * This method is an implementation of the message listener. It receives a message and places it on the queue for processing by the message listening
     * thread.
     *
     * @param data the data
    ...
    public void onMessage(final MessageBlock<Message> data) {
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("message received from client application {} port {}", data.getConnection().getRemoteSocketAddress().getAddress(),
    ...
    /**
     * This method handles EngDep messages as they come in. It uses the inevitable switch statement to handle the messages.
     *
        ...
        try {
            ...
            if (message.getAction() instanceof EngDepAction) {
                ...
                throw new ApexException(
                        message.getAction().getClass().getName() + "action on received message invalid, action must be of type \"EnDepAction\"");
            }

            // Handle each incoming message using the inevitable switch statement for the EngDep protocol
            switch (enDepAction) {
            case GET_ENGINE_SERVICE_INFO:
                ...
                sendServiceInfoReply(webSocket, engineServiceInformationMessage, apexService.getKey(), apexService.getEngineKeys(),
                        apexService.getApexModelKey());
                LOGGER.debug("returned engine service information for engine service " + apexService.getKey().getID());
                break;
            ...
            case UPDATE_MODEL:
                ...
                // Update the model
                apexService.updateModel(updateModelMessage.getTarget(), updateModelMessage.getMessageData(), updateModelMessage.isForceInstall());
                // Send a reply indicating the message action worked
                sendReply(webSocket, updateModelMessage, true, "updated model in engine " + updateModelMessage.getTarget().getID());
            ...
            case START_ENGINE:
                ...
                // Send a reply indicating the message action worked
                sendReply(webSocket, startEngineMessage, true, "started engine " + startEngineMessage.getTarget().getID());
            ...
            case STOP_ENGINE:
                ...
                // Send a reply indicating the message action worked
                sendReply(webSocket, stopEngineMessage, true, "stopped engine " + stopEngineMessage.getTarget().getID());
                LOGGER.debug("stopping engine {}", stopEngineMessage.getTarget().getID());
                break;

            case START_PERIODIC_EVENTS:
                final StartPeriodicEvents startPeriodicEventsMessage = (StartPeriodicEvents) message;
                LOGGER.debug("starting periodic events on engine {} . . .", startPeriodicEventsMessage.getTarget().getID());
                ...
                // Send a reply indicating the message action worked
                sendReply(webSocket, startPeriodicEventsMessage, true,
                        "started periodic events on engine " + startPeriodicEventsMessage.getTarget().getID() + " with period " + period);
                LOGGER.debug("started periodic events on engine " + startPeriodicEventsMessage.getTarget().getID() + " with period " + period);
                break;

            case STOP_PERIODIC_EVENTS:
                final StopPeriodicEvents stopPeriodicEventsMessage = (StopPeriodicEvents) message;
                LOGGER.debug("stopping periodic events on engine {} . . .", stopPeriodicEventsMessage.getTarget().getID());
                // Stop periodic events
                apexService.stopPeriodicEvents();
                // Send a reply indicating the message action worked
                sendReply(webSocket, stopPeriodicEventsMessage, true, "stopped periodic events on engine " + stopPeriodicEventsMessage.getTarget().getID());
            ...
            case GET_ENGINE_STATUS:
                ...
                sendReply(webSocket, getEngineStatusMessage, true, apexService.getStatus(getEngineStatusMessage.getTarget()));
     ...
     */
    private void sendReply(final WebSocket client, final Message requestMessage, final boolean result, final String messageData) {
        ...
        LOGGER.debug("sending {} to web socket {}", requestMessage.getAction(), client.getRemoteSocketAddress().toString());
     ...
     */
    private void sendServiceInfoReply(final WebSocket client, final Message requestMessage, final AxArtifactKey engineServiceKey,
            final Collection<AxArtifactKey> engineKeyCollection, final AxArtifactKey apexModelKey) {
        LOGGER.entry();
        LOGGER.debug("sending {} to web socket {}", requestMessage.getAction(), client.getRemoteSocketAddress().toString());

        final EngineServiceInfoResponse responseMessage = new EngineServiceInfoResponse(requestMessage.getTarget(), true, requestMessage);
