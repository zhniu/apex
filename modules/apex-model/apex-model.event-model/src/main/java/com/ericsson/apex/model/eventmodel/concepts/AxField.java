...
/**
 * In Apex, a field is an input or output parameter to or from a concept. For example, the parameters of an event are fields and the input and output of a task
 * is defined as a collection of fields.
 * <p>
 * A field has an {@link AxReferenceKey} key that defines its name and parent, and a {@link AxArtifactKey} key to a context schema that defines the structure of
 * the data atom that holds the value of the field. Fields can be specified as being optional but are mandatory by default.
 * <p>
...
@XmlRootElement(name = "apexField", namespace = "http://www.ericsson.com/apex")
...
public class AxField extends AxConcept {
    ...
    /**
     * Constructor to create the field with the local name of its reference key defined, its schema key and optionality defined.
     *
     * @param localName the local name of the field reference key
     * @param fieldSchemaKey the key of the field schema to use for this field
     * @param optional true if this field is optional
    ...
    public AxValidationResult validate(final AxValidationResult resultIn) {
        ...
        if (fieldSchemaKey.equals(AxArtifactKey.getNullKey())) {
            result.addValidationMessage(
                    new AxValidationMessage(key, this.getClass(), ValidationResult.INVALID, "fieldSchemaKey is a null key: " + fieldSchemaKey));
        }
