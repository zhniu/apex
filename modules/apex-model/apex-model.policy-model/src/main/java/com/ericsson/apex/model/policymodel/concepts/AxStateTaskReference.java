...
/**
 * This class defines the type of output handling that will be used when a task in a state completes execution. Each task {@link AxTask} in a state
 * {@link AxState} must select a state output {@link AxStateOutput} in order to pass its fields to an output event. Therefore, each task has an associated
 * instance of this class that defines how the state output of the state is selected and how the output fields of the task are marshaled onto the fields of the
 * output event. A {@link AxStateTaskReference} instance defines the task output handling as either {@link AxStateTaskOutputType#DIRECT} or
 * {@link AxStateTaskOutputType#LOGIC}. In the case of {@link AxStateTaskOutputType#DIRECT} output selection, the output reference key held in this
 * {@link AxStateTaskReference} instance to an instance of an {@link AxStateOutput} class. In the case of {@link AxStateTaskOutputType#LOGIC} output selection,
 * the output reference key held in this {@link AxStateTaskReference} instance to an instance of an {@link AxStateFinalizerLogic} class. See the explanation in
 * the {@link AxState} class for a full description of this handling.
 * <p>
 *
 * During validation of a state task reference, the validation checks listed below are executed:
 * <ol>
 * <li>The state task reference key must not be a null key and must be valid, see validation in {@link AxReferenceKey}
 * <li>The output type must be defined, that is not equal to {@link AxStateTaskOutputType#UNDEFINED}
 * <li>The output key must not be a null key and must be valid, see validation in {@link AxReferenceKey}
 * </ol>
 */
...
@XmlRootElement(name = "apexStateTaskReference", namespace = "http://www.ericsson.com/apex")
@XmlType(name = "AxStateTaskReference", namespace = "http://www.ericsson.com/apex", propOrder = { "key", "outputType", "output" })
...
public class AxStateTaskReference extends AxConcept {
    ...
    /**
     * The Default Constructor creates a state task reference with a null reference key, an undefined output type and a null output reference key.
     */
    ...
    /**
     * This Constructor creates a state task reference instance with a reference key composed from the given parent key with a local name composed by
     * concatenating the name of the task key with the local name of the output. The output type and output are set to the given values.
     *
     ...
     * @param outputType the type of output to perform when this state task reference instance is used
     * @param output the output to perform when this state task reference instance is used
     */
    public AxStateTaskReference(final AxReferenceKey parentKey, final AxArtifactKey taskKey, final AxStateTaskOutputType outputType,
            final AxReferenceKey output) {
        this(new AxReferenceKey(parentKey, taskKey.getName() + '_' + outputType.name() + '_' + output.getLocalName()), outputType, output);
    }

    /**
     * This Constructor creates a state task reference instance with the given reference key and all its fields defined.
     *
     * @param key the key of the state task reference
     * @param outputType the type of output to perform when this state task reference instance is used
     * @param output the output to perform when this state task reference instance is used
     */
    public AxStateTaskReference(final AxReferenceKey key, final AxStateTaskOutputType outputType, final AxReferenceKey output) {
        super();
     ...
     *
     * @param stateTaskOutputType the type of output to perform when this state task reference instance is used
     */
    ...
    public AxValidationResult validate(final AxValidationResult resultIn) {
        ...
        if (outputType == AxStateTaskOutputType.UNDEFINED) {
            result.addValidationMessage(new AxValidationMessage(key, this.getClass(), ValidationResult.INVALID, "outputType may not be UNDEFINED"));
        }

        if (output.equals(AxReferenceKey.getNullKey())) {
            result.addValidationMessage(
                    new AxValidationMessage(key, this.getClass(), ValidationResult.INVALID, "output key " + output.getID() + " is a null key"));
        }
