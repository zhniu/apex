...
/**
 * This class holds the definition of a single state in a policy. A state is a single stage in a policy. A state has a single input event, its trigger. A state
 * can output many events, but can only output one event on a single execution. After it executes, a state can pass control to another state or can simply emit
 * its event to an external system. In the case where a state passes control to another state, the output event of the state becomes the input event of the next
 * state. The outputs of a state {@link AxStateOutput} are held as a map in the state. Each state output contains the outgoing event of the state and optionally
 * the next state to pass control to.
 * <p>
 * A state uses tasks {@link AxTask} to execute its logic. A state holds its tasks in a map and must have at least one task. A state uses Task Selection Logic
 * {@link AxTaskSelectionLogic} to select which task should be executed in a given execution cycle. Optional Task Selection Logic can use fields on the incoming
 * event and information from the context albums available on the state to decide what task to execute in a given context. The default task of a state is the
 * task that is executed when task Selection Logic is not specified. In cases where only a single task is specified on a state, the default task must be that
 * task and the state always executes that task.
 * <p>
 * What happens when a state completes its execution cycle depends on the task that is selected for execution by the state. Therefore, the action to be
 * performed a state on execution of each task must be defined in the state as a {@link AxStateTaskReference} instance for each task defined in the state. The
 * {@link AxStateTaskReference} instance defines the action to be performed as either a {@link AxStateTaskOutputType} of {@link AxStateTaskOutputType#DIRECT} or
 * {@link AxStateTaskOutputType#LOGIC} and contains an {@link AxReferenceKey} reference to the instance that will complete the state output.
 * <p>
 * In the case of direct output, the {@link AxReferenceKey} reference in the {@link AxStateTaskReference} instance is a reference to an {@link AxStateOutput}
 * instance. The state output defines the event to be emitted by the state and the next state to pass control to if any. All fields of the executed task are
 * marshaled onto the outgoing event automatically by Apex.
 * <p>
 * In the case of logic output, the {@link AxReferenceKey} reference in the {@link AxStateTaskReference} instance is a reference to State Finalizer Logic in an
 * {@link AxStateFinalizerLogic} instance, which selects the {@link AxStateOutput} that the state will use. The state finalizer logic uses fields emitted by the
 * executed task and information from the context albums available on the state to decide what {@link AxStateOutput} to select in a given context. The state
 * output defines the event to be emitted by the state and the next state to pass control to if any. The State Finalizer Logic instances for the state are held
 * in a map in the state. State Finalizer Logic must marshal the fields of the output event in whatever manner it wishes; Apex does not automatically transfer
 * the output fields from the task directly to the output event.
 * <p>
 * The Task Selection Logic instance or State Finalizer Logic instances in a state may use information in context albums to arrive at their task or state output
 * selections. The context albums that the state uses and that should be made available to the state by Apex policy distribution are held as a set of references
 * to context albums in the state.
 * <p>
 * During validation of a state, the validation checks listed below are executed:
 * <ol>
 * <li>The policy key must not be a null key and must be valid, see validation in {@link AxReferenceKey}
 * <li>The trigger event key must not be a null key and must be valid, see validation in {@link AxArtifactKey}
 ...
 * <li>The default task key must not be a null key and must be valid, see validation in {@link AxArtifactKey}
 * <li>The default task must appear in the task map of the state
 * <li>At least one task must be defined on the state
 * <li>Each task key on the task map for the state must not be a null key and must be valid, see validation in {@link AxArtifactKey}
 * <li>All state task references for each task in the state must exist and must be valid, see validation in {@link AxStateTaskReference}
 ...
 * <li>An observation is issued for each state output defined on the state that is not used as a direct output on a task
 * <li>An observation is issued for each state finalizer logic instance defined on the state that is not used as an output on a task
 * <li>Each context album key on the context album set for the state must not be a null key and must be valid, see validation in {@link AxArtifactKey}
...
@XmlRootElement(name = "apexState", namespace = "http://www.ericsson.com/apex")
@XmlType(name = "AxState", namespace = "http://www.ericsson.com/apex", propOrder = { "key", "trigger", "stateOutputs", "contextAlbumReferenceSet",
...
public class AxState extends AxConcept {
    ...
    @JoinTable(joinColumns = {
            @JoinColumn(name = "soParentKeyName", referencedColumnName = "parentKeyName"),
    ...
    inverseJoinColumns = {
            @JoinColumn(name = "stateParentKeyName", referencedColumnName = "parentKeyName"),
    ...
    @JoinTable(name = "STATE_TSL_JT", joinColumns = {
            @JoinColumn(name = "tslParentKeyName", referencedColumnName = "parentKeyName", updatable = false, insertable = false),
            @JoinColumn(name = "tslParentKeyVersion", referencedColumnName = "parentKeyVersion", updatable = false, insertable = false),
            @JoinColumn(name = "tslParentLocalName ", referencedColumnName = "parentLocalName", updatable = false, insertable = false),
            @JoinColumn(name = "tslLocalName", referencedColumnName = "localName", updatable = false, insertable = false)
    })
    @XmlElement(required = true)
    private AxTaskSelectionLogic taskSelectionLogic;

    @ManyToMany(cascade = CascadeType.ALL)
    @JoinTable(joinColumns = { @JoinColumn(name = "sflParentKeyName", referencedColumnName = "parentKeyName"),
    ...
    @ManyToMany(cascade = CascadeType.ALL)
    @JoinTable(joinColumns = { @JoinColumn(name = "trmParentKeyName", referencedColumnName = "parentKeyName"),
     ...
     * @param contextAlbumReferenceSet the context album reference set defines the context that may be used by Task Selection Logic and State Finalizer Logic in
     *            the state
     * @param taskSelectionLogic the task selection logic that selects the task a state executes in an execution cycle
     * @param stateFinalizerLogicMap the state finalizer logic instances that selects the state output to use after a task executes in a state execution cycle
     * @param defaultTask the default task that will execute in a state if Task Selection Logic is not specified
     * @param taskReferenceMap the task reference map that defines the tasks for the state and how the task outputs are handled
     */
    ...
    /**
     * When a state is unmarshalled from disk or from the database, the parent of contained objects is not defined. This method is called by JAXB after
     * unmarshaling and is used to set the parent keys of all {@link AxTaskSelectionLogic}, {@link AxStateOutput}, and {@link AxStateFinalizerLogic} instance in
     * the state.
     *
    ...
    public List<AxKey> getKeys() {
        ...
        for (final Entry<AxArtifactKey, AxStateTaskReference> taskReferenceEntry : taskReferenceMap.entrySet()) {
            ...
            // A state output is allowed to be used more than once but we only return one usage as a key
    ...
    /**
     * Gets the context album reference set defines the context that may be used by Task Selection Logic and State Finalizer Logic in the state.
     *
     * @return the context album reference set defines the context that may be used by Task Selection Logic and State Finalizer Logic in the state
     */
    ...
    /**
     * Sets the context album reference set defines the context that may be used by Task Selection Logic and State Finalizer Logic in the state.
     *
     * @param contextAlbumReferences the context album reference set defines the context that may be used by Task Selection Logic and State Finalizer Logic in
     *            the state
     */
    ...
    /**
     * Gets the state finalizer logic instances that selects the state output to use after a task executes in a state execution cycle.
     *
     * @return the state finalizer logic instances that selects the state output to use after a task executes in a state execution cycle
     */
    ...
    /**
     * Sets the state finalizer logic instances that selects the state output to use after a task executes in a state execution cycle.
     *
     * @param stateFinalizerLogicMap the state finalizer logic instances that selects the state output to use after a task executes in a state execution cycle
     */
     ...
     *
     * @param taskReferences the task reference map that defines the tasks for the state and how the task outputs are handled
     */
    ...
    public AxValidationResult validate(final AxValidationResult resultIn) {
        ...
        if (trigger.equals(AxArtifactKey.getNullKey())) {
            result.addValidationMessage(new AxValidationMessage(key, this.getClass(), ValidationResult.INVALID, "trigger is a null key: " + trigger));
        }
        result = trigger.validate(result);

        if (stateOutputs.size() == 0) {
            result.addValidationMessage(new AxValidationMessage(key, this.getClass(), ValidationResult.INVALID, "stateOutputs may not be empty"));
        }
        else {
            ...
            for (final Entry<String, AxStateOutput> stateOutputEntry : stateOutputs.entrySet()) {
                if (stateOutputEntry.getValue() == null) {
                    ...
                    if (stateOutputEntry.getValue().getNextState().getLocalName().equals(key.getLocalName())) {
                        result.addValidationMessage(new AxValidationMessage(key, this.getClass(), ValidationResult.INVALID,
                                "state output next state " + stateOutputEntry.getValue().getNextState().getLocalName() + " may not be this state"));

                    }

                    if (nextStateNameSet.contains(stateOutputEntry.getValue().getNextState().getLocalName())) {
                        result.addValidationMessage(new AxValidationMessage(key, this.getClass(), ValidationResult.INVALID,
                                "duplicate state output next state name " + stateOutputEntry.getValue().getNextState().getLocalName() + " found"));
                    }
                    else {
        ...
        for (final AxArtifactKey contextAlbumReference : contextAlbumReferenceSet) {
            if (contextAlbumReference.equals(AxArtifactKey.getNullKey())) {
                result.addValidationMessage(new AxValidationMessage(key, this.getClass(), ValidationResult.INVALID,
                        "key on context album reference entry " + contextAlbumReference.getKey() + " may not be the null key"));
            }
        ...
        for (final Entry<String, AxStateFinalizerLogic> stateFinalizerLogicEntry : stateFinalizerLogicMap.entrySet()) {
            if (stateFinalizerLogicEntry.getValue() == null) {
                result.addValidationMessage(new AxValidationMessage(key, this.getClass(), ValidationResult.INVALID,
                        "null state finalizer logic value found on state finalizer entry " + stateFinalizerLogicEntry.getKey()));
            }
            else {
        ...
        if (defaultTask.equals(AxArtifactKey.getNullKey())) {
            result.addValidationMessage(new AxValidationMessage(key, this.getClass(), ValidationResult.INVALID, "default task has a null key: " + defaultTask));
        }
        result = defaultTask.validate(result);

        if (taskReferenceMap.size() == 0) {
            result.addValidationMessage(new AxValidationMessage(key, this.getClass(), ValidationResult.INVALID, "taskReferenceMap may not be empty"));
        }
        else {
            ...
            for (final Entry<AxArtifactKey, AxStateTaskReference> taskRefEntry : taskReferenceMap.entrySet()) {
                if (taskRefEntry.getKey().equals(AxArtifactKey.getNullKey())) {
                    result.addValidationMessage(
                            new AxValidationMessage(key, this.getClass(), ValidationResult.INVALID, "task has a null key: " + taskRefEntry.getKey()));
                }
                ...
                if (taskRefEntry.getValue() == null) {
                    ...
                    if (taskRefEntry.getValue().getStateTaskOutputType().equals(AxStateTaskOutputType.DIRECT)) {
                        if (stateOutputs.containsKey(taskRefEntry.getValue().getOutput().getLocalName())) {
                            usedStateOutputNameSet.add(taskRefEntry.getValue().getOutput().getLocalName());
                        }
                        else {
                            result.addValidationMessage(new AxValidationMessage(key, this.getClass(), ValidationResult.INVALID,
                                    "state output for task " + taskRefEntry.getKey() + " not found in stateOutputs"));
                        }
                    }
                    else if (taskRefEntry.getValue().getStateTaskOutputType().equals(AxStateTaskOutputType.LOGIC)) {
                        if (stateFinalizerLogicMap.containsKey(taskRefEntry.getValue().getOutput().getLocalName())) {
                            usedStateFinalizerLogicNameSet.add(taskRefEntry.getValue().getOutput().getLocalName());
                        }
                        else {
                            result.addValidationMessage(new AxValidationMessage(key, this.getClass(), ValidationResult.INVALID,
                                    "state finalizer logic for task " + taskRefEntry.getKey() + " not found in stateFinalizerLogicMap"));
                        }
                    }
                    else {
                        result.addValidationMessage(new AxValidationMessage(key, this.getClass(), ValidationResult.INVALID,
                                "stateTaskReference task output type " + taskRefEntry.getValue().getStateTaskOutputType() + " is invalid"));
                    }

                    result = taskRefEntry.getValue().validate(result);
                }
            }
