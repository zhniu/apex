...
public class AxState extends AxConcept {
	...
	private void validateStateTaskReferences(AxValidationResult result) {
		...
		for (final Entry<AxArtifactKey, AxStateTaskReference> taskRefEntry : taskReferenceMap.entrySet()) {
			...
			if (taskRefEntry.getValue() == null) {
				...
				result = validateStateTaskReference(taskRefEntry.getKey(), taskRefEntry.getValue(), usedStateOutputNameSet, usedStateFinalizerLogicNameSet, result);
			}
		}
	 ...
	 * @param stateFinalizerLogicNameSet State finalizers that have been used so far, may be appended if this task reference uses a finalzier 
	 * @param result the validation result to append to
	 * @return the result of the validation
	 */
	private AxValidationResult validateStateTaskReference(final AxArtifactKey taskKey, final AxStateTaskReference taskReference,
			Set<String> stateOutputNameSet, Set<String> stateFinalizerLogicNameSet, AxValidationResult result) {
