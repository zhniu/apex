...
/**
 * The Class is used to return the tree that represents the state branches or chains in a policy. It creates a tree that holds the state fan out branches in a
 * policy that starts from the given top state of the tree. Each branch from a state is held in a set of next states for the top state and each branch in the
 * state tree is itself a {@link AxStateTree} instance.
 * <p>
 * Validation checks for recursive state use, in other words validation forbids the use of a given state more than once in a state tree.
 */
public class AxStateTree implements Comparable<AxStateTree> {
    ...
    /**
     * This constructor recursively creates a state tree for the given policy starting at the given state.
     *
     * @param policy the policy from which to read states
     * @param thisState the state to start state tree construction at
     * @param referencedStateNameSet a set of state names already referenced in the tree, null for the first recursive call
     */
    public AxStateTree(final AxPolicy policy, final AxState thisState, Set<AxReferenceKey> referencedStateNameSet) {
        ...
        for (final AxStateOutput stateOutput : thisState.getStateOutputs().values()) {
            ...
            if (referencedStateNameSet.contains(nextState.getKey())) {
                throw new PolicyRuntimeException("loop detected in state tree for policy " + policy.getID() +
