...
/**
 * This class defines a single output that a state can have. A state can have many outputs with each output defined as an instance of this class. Each state
 * output defines the output event that will be emitted when this output is selected and optionally the next state that is executed when this state output is
 * selected. If no next state is defined (the next state is a null {@link AxReferenceKey} key), then this state output outputs its event to an external system
 * and is an output state for the full policy.
 * <p>
 * During validation of a state output, the validation checks listed below are executed:
 * <ol>
 * <li>The state output key must not be a null key and must be valid, see validation in {@link AxReferenceKey}
 * <li>The outgoing event key must not be a null key and must be valid, see validation in {@link AxArtifactKey}
...
@XmlRootElement(name = "apexStateOutput", namespace = "http://www.ericsson.com/apex")
@XmlType(name = "AxStateOutput", namespace = "http://www.ericsson.com/apex", propOrder = { "key", "outgoingEvent", "nextState" })
...
public class AxStateOutput extends AxConcept {
    ...
    /**
     * The Default Constructor creates a state output instance with a null reference key, outgoing event key and next state reference key.
     */
    ...
    /**
     * This Constructor creates a state output with a reference key composed of the given parent key and with a local name composed from the parent key local
     * name concatenated with the next state's local name. The next state and outgoing event of the state output are set as specified.
     *
     ...
     */
    public AxStateOutput(final AxReferenceKey parentKey, final AxReferenceKey nextState, final AxArtifactKey outgoingEvent) {
        this(new AxReferenceKey(parentKey, parentKey.getLocalName() + '_' + nextState.getLocalName()), outgoingEvent, nextState);
    }

    /**
     * This Constructor creates a state output with the specified reference key. The next state and outgoing event of the state output are set as specified.
     *
     * @param key the key
     * @param nextState the next state to which execution will pass on use of this state output
     * @param outgoingEvent the outgoing event emitted on use of this state output
