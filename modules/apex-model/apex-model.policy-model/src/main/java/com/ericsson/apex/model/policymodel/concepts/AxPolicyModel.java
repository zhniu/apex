...
/**
 * A container class for an Apex policy model. This class is a container class that allows an Apex model to be constructed that contains definitions of all the
 * context, event, and policy concepts required to run policies in Apex. The model contains schema definitions, definitions of events and context albums that
 * use those schemas, definitions of tasks for policies and definitions of the policies themselves.
 * <p>
 * An Apex policy model is an important artifact in Apex. At editing time, an Apex editor creates and edits a policy model and a policy model is loaded into and
 * is executed by an Apex engine. Therefore, an Apex model and the set of policies that it holds is the way that the policy domain that an Apex engine or a
 * group of Apex engines executes across is expressed, both at design time and run time. The Apex deployment system is responsible for deploying Apex models to
 * and the context they need the appropriate engines for execution.
 * <p>
 * Model registration is carried out by calling the {@code register()} method, which registers the policy model and all its constituent containers with the
 * model service. The containers for context schemas, events, context albums, tasks, policies, and key information are all registered.
 * <p>
 * Once a policy model is composed, the overall structure of the policy model and all its references can be validated. During validation of a policy model, the
 * validation checks listed below are executed:
 * <ol>
 * <li>The policy model is validated as an Apex model, which validates the existence, correctness, and duplication of all keys in the model as well as
 * validating the key information of the keys, see validation in {@link AxModel}
 ...
 * <li>In a state, an event that triggers a task must contain all the input fields required by that task
 * <li>In a state, an event that is emitted by a task must contain all the output fields produced by that task
...
@XmlRootElement(name = "apexPolicyModel", namespace = "http://www.ericsson.com/apex")
...
public class AxPolicyModel extends AxModel {
    ...
    /**
     * The Default Constructor creates a policy model with a null key and empty containers for schemas, key information, events, context albums, tasks and
     * policies.
     */
     ...
     */
    public AxPolicyModel(final AxArtifactKey key, final AxContextSchemas schemas, final AxKeyInformation keyInformation, final AxEvents events,
            final AxContextAlbums albums, final AxTasks tasks, final AxPolicies policies) {
    ...
    public AxContextModel getContextModel() {
        return new AxContextModel(new AxArtifactKey(albums.getKey().getName() + "_Model", albums.getKey().getVersion()), getSchemas(), getAlbums(),
                getKeyInformation());
    }
