...
/**
 * A container class for an Apex context model. This class is a container class that allows an Apex model to be constructed that just contains context and the
 * key information for that context. The model contains schema definitions and the definitions of context albums that use those schemas. In the case where Apex
 * context is being used without policy or independent of policy, an Apex context model is sufficient to get Apex context working.
 * <p>
 * Validation runs {@link AxModel} validation on the model. In addition, the {@link AxContextSchemas} and {@link AxContextAlbums} validation is run on the
 * context schemas and albums in the model.
 */
@Entity
...
public class AxContextModel extends AxModel {
    ...
    /**
     * The Default Constructor creates a {@link AxContextModel} object with a null artifact key and creates an empty context model.
     */
    ...
    /**
     * The Key Constructor creates a {@link AxContextModel} object with the given artifact key and creates an empty context model.
     *
     ...
     */
    public AxContextModel(final AxArtifactKey key, final AxContextSchemas schemas, final AxKeyInformation keyInformation) {
        this(key, schemas, new AxContextAlbums(new AxArtifactKey(key.getName() + "_Albums", key.getVersion())), keyInformation);
    }
     ...
     * @param keyInformation the key information for those context schemas
     */
    public AxContextModel(final AxArtifactKey key, final AxContextSchemas schemas, final AxContextAlbums albums, final AxKeyInformation keyInformation) {
