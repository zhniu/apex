...
----
include::{apex-domains-myfirstpolicy-dir}/main/resources/examples/models/MyFirstPolicy/1/MorningBoozeCheck.mvel[`MorningBoozeCheck` task logic (`MVEL`)]
----
...
_An alternative version of the same logic is available in JavaScript_ link:files/mfp-files/1/MorningBoozeCheck.js[MorningBoozeCheck.js]_. Just use "JAVASCRIPT" as the 'Task Logic Type / Flavour' instead._
...
Congratulations, you have now completed your first Apex policy. The policy model containing our new policy can now be exported from the editor and saved. Click on the 'File' menu and select 'Download' to save the policy model in JSON format.  The exported policy model is then available in the directory you selected, as link:files/mfp-files/1/MyFirstPolicyModel_0.0.1.json[_MyFirstPolicyModel_0.0.1.json_]. The exported policy can now be loaded into the Apex Policy Engine, or can be re-loaded and edited by the Apex Policy Editor.
