...
import com.ericsson.apex.domains.adaptive.concepts.AnomalyDetection;
...
public class AnomalyDetectionPolicy_Decide_TaskSelectionLogic {
 // CHECKSTYLE:ON: checkstyle:className
    ...
    public boolean getTask(final TaskSelectionExecutionContext executor) {
        ...
        final double[] vals = this.forecastingAndAnomaly(now); // double[forecastedValue, AnomalyScore, AnomalyProbability]
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("TestAnomalyDetectionTSLPolicy0000DecideStateTaskSelectionLogic.getTask():\t************\t\t\t\t" + "Iteration:\t" + iteration
                    + "\tValue:\t" + now + "\tForecast:\t" + vals[0] + "\tAnomalyScore:\t" + vals[1] + "\tAnomalyProbability:\t" + vals[2]
                    + "\tInvoking Task:\t" + executionContext.selectedTask);
        }
        return true;
    }
     ...
     * @return Null if the function can not be executed correctly, otherwise double[forecastedValue, AnomalyScore, AnomalyProbability]
     */
    public double[] forecastingAndAnomaly(final double value) {
        ...
        AnomalyDetection anomalyDetection = (AnomalyDetection) executionContext.getContextAlbum("AnomalyDetectionAlbum").get("AnomalyDetection");
     ...
     * @return true if the value is between interval[0] (inclusive) and interval[1] (exclusive), i.e. (value<interval[1] && value>=interval[0]). Otherwise
     *         false;
     */
