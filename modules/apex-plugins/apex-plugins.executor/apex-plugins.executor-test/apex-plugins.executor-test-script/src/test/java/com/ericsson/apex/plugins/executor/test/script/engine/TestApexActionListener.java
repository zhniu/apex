 ...
 * the object created with that class is registered with a component using the component's <code>addTestApexActionListener</code> method. When
 * the testApexAction event occurs, that object's appropriate
 * method is invoked.
 *
...
public class TestApexActionListener implements EnEventListener {
	 ...
	 * @param id the id
	 */
	public TestApexActionListener(String id) {
		this.id = id;
	}
	 ...
	 * Gets the result.
	 *
	 * @param allowNulls if true and the returned event is null, then return, otherwise wait until an event is returned.
	 * @return the result
	 */
	public EnEvent getResult(boolean allowNulls) {
		EnEvent result = null;
		while (true) {
	 ...
	 */
	@Override
	public void onEnEvent(EnEvent actionEvent) {
		try {
			Thread.sleep(100);
		}
