...
public class KAFKACarrierTechnologyParameters extends CarrierTechnologyParameters {
    ...
    /**
     * Constructor to create a kafka carrier technology parameters instance and register the instance with the parameter service.
     */
        ...
        for (final String consumerTopic : consumerTopicList) {
            if (consumerTopic == null || consumerTopic.trim().length() == 0) {
                errorMessageBuilder.append(
                        "  invalid consumer topic \"" + consumerTopic + "\" specified on consumerTopicList, consumer topics must be specified as strings\n");
            }
        }
