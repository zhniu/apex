 ...
 * <li>It creates an {@link com.ericsson.apex.service.engine.event.ApexEvent} instance to hold the java object.
 * <li>It sets the name of the Apex event to be the simple class name of the incoming Java object and appends the value of the {@code incomingEventSuffix}
 * parameter to it.
 * <li>It sets the version of the incoming event to the value of the {@code incomingEventVersion} parameter.
 * <li>It sets the name space of the incoming event to be the value of the package of the class of the incoming Java object.
 ...
 * <li>It puts a single entry into the Apex event map with the the simple class name of the incoming Java object being the key of the entry and the actual
 * incoming object as the value of the entry.
 * </ol>
 * <p>
 * When sending an object to JMS, the plugin expects to receive an Apex event with a single entry. The plugin marshals the value of that entry to an object that
 * can be sent by JMS as a {@link javax.jms.ObjectMessage} instance.
 * <p>
 * The parameters for this plugin are:
 * <ol>
 * <li>incomingEventSuffix: The suffix to append to the simple name of incoming Java class instances when they are encapsulated in Apex events. The parameter
 * defaults to the string value {@code IncomingEvent}.
 * <li>incomingEventVersion: The event version to use for incoming Java class instances when they are encapsulated in Apex events. The parameter defaults to the
 * string value {@code 1.0.0}.
 * <li>incomingEventSource: The event source to use for incoming Java class instances when they are encapsulated in Apex events. The parameter defaults to the
 * string value {@code JMS}.
 * <li>incomingEventTarget: The event target to use for incoming Java class instances when they are encapsulated in Apex events. The parameter defaults to the
 * string value {@code Apex}.
 * </ol>
 * 
