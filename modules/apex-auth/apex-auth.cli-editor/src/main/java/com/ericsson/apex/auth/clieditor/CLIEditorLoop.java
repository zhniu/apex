...
/**
 * This class implements the editor loop, the loop of execution that continuously executes commands until the quit command is issued or EOF is detected on
 * input.
 *
...
public class CLIEditorLoop {
     ...
     */
    public CLIEditorLoop(final Properties properties, final ApexModelHandler modelHandler, final KeywordNode rootKeywordNode) {
    ...
    public int runLoop(final InputStream inputStream, final OutputStream outputStream, final CLIParameters parameters) throws IOException {
        ...
        while (result.getResult() != RESULT.FINISHED) {
            ...
            try {
                ...
                // Find the command, if the command is null, then we are simply changing position in the hierarchy
    ...
    /**
     * Finds a command for the given input command words. Command words need only ne specified enough to uniquely identify them. Therefore, "p s o c" will find
     * the command "policy state output create"
     *
    ...
    private CLICommand findCommand(final ArrayList<String> commandWords) {
        ...
        for (int i = 0; i < commandWords.size(); i++) {
            ...
            // If the node entries found is not equal to one, then we have either no command or more than one command matching
            final List<Entry<String, KeywordNode>> foundNodeEntries = TreeMapUtils.findMatchingEntries(searchKeywordNode.getChildren(), commandWords.get(i));
            if (foundNodeEntries.size() == 0) {
                ...
                unwindStack(startKeywordNode);
                throw new CLIException("multiple commands matched: " + stringAL2String(commandWords) + " [" + nodeAL2String(foundNodeEntries) + ']');
            }
            ...
            // If the command is null, we go into a sub mode, otherwise we unwind the stack of commands and return the found command
            if (command == null) {
     ...
     */
    private TreeMap<String, CLIArgumentValue> getArgumentValues(final CLICommand command, final ArrayList<String> commandWords) {
        ...
        for (final Entry<String, String> argument : getCommandArguments(commandWords)) {
            final List<Entry<String, CLIArgumentValue>> foundArguments = TreeMapUtils.findMatchingEntries(argumentValues, argument.getKey());
            if (foundArguments.size() == 0) {
                throw new CLIException("command " + stringAL2String(commandWords) + ": " + " argument \"" + argument.getKey() + "\" not allowed on command");
            }
            else if (foundArguments.size() > 1) {
                throw new CLIException("command " + stringAL2String(commandWords) + ": " + " argument " + argument + " matches multiple arguments ["
                        + argumentAL2String(foundArguments) + ']');
            }
        ...
        for (final CLIArgumentValue argumentValue : argumentValues.values()) {
            if (!argumentValue.isSpecified()) {
                // Argument values are null by default so if this argument is not nullable it is mandatory
    ...
    /**
     * Get the arguments of the command, the command words have already been conditioned into an array starting with the command words and ending with the
     * arguments as name=value tuples.
     *
    ...
    private ArrayList<Entry<String, String>> getCommandArguments(final ArrayList<String> commandWords) {
        ...
        for (final String word : commandWords) {
            ...
            if (equalsPos > 0) {
                arguments.add(new SimpleEntry<>(word.substring(0, equalsPos), word.substring(equalsPos + 1, word.length())));
            }
        }
    ...
    private String expandMacroFile(CLIParameters parameters, final String line) {
        ...
        String lineBeforeMacroTag = line.substring(0, macroTagPos);
        String lineAfterMacroTag = line.substring(macroTagPos + macroFileTag.length()).replaceAll("^\\s*", "");

        // Get the file name that is the argument of the Macro tag
        String[] lineWords = lineAfterMacroTag.split("\\s+");
        ...
        String lineAfterMacroFileName = lineAfterMacroTag.replaceFirst(macroFileName, "");
