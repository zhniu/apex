...
public class CLILineParser {
    ...
    /**
     * This method breaks a line of input up into commands, parameters, and arguments. Commands are standalone words at the beginning of the line, of which
     * there may be multiple Parameters are single words followed by an '=' character Arguments are single words or a block of quoted text following an '='
     * character.
     *
    ...
    public ArrayList<String> parse(final String line, final String logicBlock) {
        return checkFormat(mergeArguments(mergeEquals(splitOnEquals(stripAndSplitWords(mergeQuotes(splitOnChar(stripComments(line), '\"')))))), logicBlock);
    }
    ...
    /**
     * This method merges an array with separate quotes into an array with quotes delimiting the start and end of quoted words Example [Humpty ],["],[Dumpty sat
     * on the wall],["],[, Humpty Dumpty had ],["],["],a ["],[great],["],[ fall] becomes [Humpty ],["Dumpty sat on the wall"],[, Humpty Dumpty had
     * ],[""],[a],["great"],[ fall].
     *
    ...
    /**
     * This method splits the words on an array list into an array list where each portion of the line is split into words by '=', quoted words are ignored
     * Example: aaa = bbb = ccc=ddd=eee = becomes [aaa ],[=],[bbb ],[=],[ccc],[=],[ddd],[=],[eee ],[=].
     *
    ...
    /**
     * This method merges an array with separate equals into an array with equals delimiting the start of words Example: [aaa ],[=],[bbb
     * ],[=],[ccc],[=],[ddd],[=],[eee ],[=] becomes [aaa ],[= bbb ],[= ccc],[=ddd],[=eee ],[=].
     *
    ...
    /**
     * This method merges words that start with an '=' character with the previous word if that word does not start with an '='.
     *
     * @param words the words
     * @return the merged array list
     */
    ...
    /**
     * This method strips all non quoted white space down to single spaces and splits non-quoted words into separate words.
     *
     * @param words the words
    ...
    private ArrayList<String> stripAndSplitWords(final ArrayList<String> words) {
        ...
        for (String word : words) {
            ...
            // Strip white space by replacing all white space with blanks and then removing leading and trailing blanks
    ...
    /**
     * This method splits a line of text into an array list where each portion of the line is split into words by a character, with the characters themselves as
     * separate words Example: Humpty "Dumpty sat on the wall", Humpty Dumpty had ""a "great" fall becomes [Humpty ],["],[Dumpty sat on the wall],["],[, Humpty
    ...
    private ArrayList<String> checkFormat(final ArrayList<String> commandWords, final String logicBlock) {
        ...
        if (!commandWords.get(0).matches("^[a-zA-Z0-9]*$")) {
            throw new CLIException("first command word is not alphanumeric or is not a command: " + commandWords.get(0));
        }
        ...
        while (currentWordPos < commandWords.size()) {
            // From now on we should have a sequence of parameters with arguments delimited by a single '=' character
            if (currentWordPos < commandWords.size() - 1 || logicBlock == null) {
                ...
                if (commandWords.get(currentWordPos).matches("^[a-zA-Z0-9]+=[a-zA-Z0-9/\"].*$")) {
                    ...
                    throw new CLIException("command argument is not properly formed: " + commandWords.get(currentWordPos));
                }
            }
            else {
                // Logic block
