...
public class CLIParameterParser {
    ...
    public CLIParameterParser() {
        ...
        options.addOption(Option.builder("m").longOpt("metadata-file").desc("name of the command metadata file to use").hasArg().argName("CMD_METADATA_FILE")
                .required(false).type(String.class).build());
        options.addOption(Option.builder("a").longOpt("model-props-file").desc("name of the apex model properties file to use").hasArg()
                .argName("MODEL_PROPS_FILE").required(false).type(String.class).build());
        options.addOption(Option.builder("c").longOpt("command-file").desc("name of a file containing editor commands to run into the editor").hasArg()
                .argName("COMMAND_FILE").required(false).type(String.class).build());
        options.addOption(Option.builder("l").longOpt("log-file").desc(
                "name of a file that will contain command logs from the editor, will log to standard output if not specified or suppressed with \"-nl\" flag")
                .hasArg().argName("LOG_FILE").required(false).type(String.class).build());
        options.addOption(
                Option.builder("nl").longOpt("no-log").desc("if specified, no logging or output of commands to standard output or log file is carried out")
        ...
        options.addOption(Option.builder("nm").longOpt("no-model-output")
                ...
                .required(false).type(Boolean.class).build());
        options.addOption(Option.builder("i").longOpt("input-model-file").desc("name of a file that contains an input model for the editor").hasArg()
