...
public class TestFileMacro {
    ...
    public void testLogicBlock() throws IOException, ApexModelException {
        new ApexCLIEditorMain(fileMacroArgs);
        ...
        final String outputLogString = TextFileUtils.getTextFileAsString(outputLogFile.getCanonicalPath()).replaceAll("\\s+", "");

        // We compare the log to what we expect to get
        final String outputLogCompareString = TextFileUtils.getTextFileAsString("src/test/resources/compare/FileMacro_Compare.log").replaceAll("\\s+", "");
